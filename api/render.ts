...
export default async function handler(req: Request): Promise<Response> {
  ...
  // ── /library/search ──
    ...
    desc  = 'ابحث في جميع أقسام المكتبة القانونية الرقمية — القوانين، القواعد القضائية، تعليمات النيابة، واللوائح اليمنية في مكان واحد.';
    body  = `
  ...
  // ── /library/judicial ──
    ...
    desc  = 'أكثر من 1997 قاعدة قضائية ومبدأ صادر عن المحكمة العليا اليمنية — مصنّفة ومفهرسة بدوائر قضائية متخصصة على منصة الناصر.';
    body  = `
      <h1>القواعد القضائية للمحكمة العليا</h1>
      <p>${desc}</p>
      <h2>الدوائر القضائية</h2>
      <ul>
        <li>الدائرة المدنية</li>
        <li>الدائرة التجارية</li>
        <li>الدائرة الجزائية</li>
        <li>دائرة الأحوال الشخصية</li>
        <li>دائرة الإدارية والدستورية</li>
      </ul>
  ...
  // ── /library/judicial/:circuit ──
  else if (pathname.match(/^\/library\/judicial\/[\w-]+$/)) {
    title = 'قواعد قضائية | المحكمة العليا | منصة الناصر';
    desc  = 'قواعد قضائية ومبادئ صادرة عن دائرة متخصصة في المحكمة العليا اليمنية — منصة الناصر القانونية.';
    body  = `<h1>${e(title)}</h1><p>${e(desc)}</p><p><a href="${BASE_URL}/library/judicial" style="color:#1d4ed8;font-weight:700">← القواعد القضائية</a></p>`;
  }

  // ── /library/:category (تشريعات، نيابة، لوائح…) ──
  else if (pathname.match(/^\/library\/(?!doc\/|files|favorites|subscription|other-services|search)[\w-]+$/)) {
    const category = pathname.split('/').pop()!;
    const categoryNames: Record<string, { name: string; desc: string }> = {
      legislation: { name: 'القوانين اليمنية', desc: 'القوانين والتشريعات اليمنية بآخر التعديلات الرسمية — منصة الناصر القانونية.' },
      prosecution: { name: 'تعليمات النيابة العامة', desc: 'تعليمات النيابة العامة والملفات والتعليمات الجزائية الرسمية — منصة الناصر.' },
      regulations: { name: 'اللوائح والأنظمة', desc: 'اللوائح التنفيذية والأنظمة الإدارية اليمنية — منصة الناصر القانونية.' },
    };
    const cat = categoryNames[category];
    title = cat ? `${cat.name} | المكتبة القانونية | منصة الناصر` : 'المكتبة القانونية | منصة الناصر';
    desc  = cat ? cat.desc : 'تصفح وثائق المكتبة القانونية الرقمية على منصة الناصر.';
    body  = `
      <h1>${e(title)}</h1>
      <p>${e(desc)}</p>
      <p style="margin-top:16px"><a href="${BASE_URL}/library" style="color:#1d4ed8;font-weight:700">← العودة للمكتبة</a></p>`;
  ...
  // ── /library/doc/:id ──
    ...
    desc  = 'اطلع على الوثيقة القانونية كاملة في المكتبة القانونية الرقمية — منصة الناصر القانونية اليمنية.';
  ...
  // ── /library/other-services ──
    ...
    desc  = 'خدمات إضافية في المكتبة القانونية — فهارس المحكمة العليا والمراجع القانونية على منصة الناصر.';
    body  = `
  ...
  // ── /library/other-services/supreme-court ──
    ...
    desc  = 'الفهرس الشامل لأحكام ومبادئ المحكمة العليا اليمنية — منصة الناصر القانونية.';
  ...
  // ── /library/other-services/legal-references ──
    ...
    desc  = 'مراجع قانونية متخصصة للباحثين والمحامين وطلاب الشريعة والقانون — منصة الناصر.';
    body  = `<h1>${e(title)}</h1><p>${e(desc)}</p><p><a href="${BASE_URL}/library/other-services" style="color:#1d4ed8;font-weight:700">← الخدمات الأخرى</a></p>`;
  }
  ...
  // ── /library/subscription ──
    ...
    desc  = 'اشترك في المكتبة القانونية الرقمية للوصول الكامل لجميع الوثائق والتشريعات والقواعد القضائية — منصة الناصر.';
    body  = `<h1>${e(title)}</h1><p>${e(desc)}</p><p><a href="${BASE_URL}/library" style="color:#1d4ed8;font-weight:700">← العودة للمكتبة</a></p>`;
    schema = {
      '@context': 'https://schema.org',
      '@type': 'Product',
