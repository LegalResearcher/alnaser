 ...
 * يلتقط الحالات التي يلصق فيها pdf.js العلامة بالنص (".3نص"، "3. نص").
 */
function splitRowAtGutter(ws: ParserWord[], gutterX: number, pageW: number): ParserWord[][] {
  ...
  // الإصلاح A: كشف بداية علامة الخيار (تلتقط العلامة الملصوقة بالنص أيضاً)
  // نطبّقه على العمود الأيسر فقط (بداية القراءة RTL = أعلى x فيه)
 ...
 * مثال: optsText[1] = "نص أول 2. نص ثانٍ 3. نص ثالث" → يفصلها إلى 1 و2 و3.
 */
function redistributeInlineMarkers(
  ...
  for (const n of [1, 2, 3]) {
    const t = optsText[n];
    if (!t) continue;
    // ابحث عن علامة لاحقة (n+1 ..) تتبعها مسافة داخل النص
    const re = /\s([2-4])\s*[.)،]\s+/;
    const m = t.match(re);
    if (!m) continue;
...
export function parseQuestionsFromWords(
  ...
  const saveQ = () => {
    // الإصلاح B: فصل علامات الخيارات الداخلية قبل بناء السؤال
