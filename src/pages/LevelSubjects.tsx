...
const LevelSubjects = () => {
  ...
  return (
    <MainLayout>
      ...
      <section className="py-8 md:py-24 bg-slate-50 dark:bg-muted/30 min-h-[calc(100vh-80px)]">
        <div className="container mx-auto px-4 md:px-6">
          
          {/* زر العودة بتصميم عصري */}
          <button 
            onClick={() => navigate('/levels')}
            className="group inline-flex items-center gap-2 text-slate-400 dark:text-muted-foreground hover:text-primary mb-8 md:mb-12 transition-all font-bold text-xs uppercase tracking-widest active:scale-95"
          >
          ...
          <div className="max-w-4xl mb-10 md:mb-16 animate-in fade-in slide-in-from-right duration-700">
            {isLoading ? (
              <div className="space-y-4">
                ...
                <div className="h-5 md:h-6 w-72 md:w-96 bg-slate-100 dark:bg-muted animate-pulse rounded-xl" />
              </div>
            ) : (
              <>
                ...
                <h1 className="text-3xl md:text-6xl font-black text-slate-900 dark:text-foreground mb-4 md:mb-6 tracking-tight">
                  مواد <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600 italic">
                    {level?.name}
                  </span>
                </h1>
                <p className="text-slate-500 dark:text-muted-foreground text-base md:text-xl max-w-2xl leading-relaxed font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
            {isLoading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-52 md:h-64 rounded-[2rem] md:rounded-[2.5rem] bg-white dark:bg-card border border-slate-100 dark:border-border shadow-sm animate-pulse" />
              ))
            ) : subjects.length === 0 ? (
              <div className="col-span-full py-16 md:py-24 text-center bg-white dark:bg-card rounded-[2rem] md:rounded-[3rem] border border-dashed border-slate-200 dark:border-border">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-50 dark:bg-muted rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
                  <BookOpen className="w-8 h-8 md:w-10 md:h-10 text-slate-300" />
                </div>
                <p className="text-slate-500 dark:text-muted-foreground text-lg md:text-xl font-bold">لا توجد مواد في هذا المستوى حالياً</p>
                <p className="text-slate-400 dark:text-muted-foreground text-sm mt-2">سيتم إضافة المحتوى قريباً من قبل الإدارة</p>
              </div>
            ) : (
              subjects.map((subject) => (
                <div key={subject.id} className="group relative">
                  <button
                    ...
                    className="absolute top-4 left-4 z-20 w-9 h-9 rounded-xl bg-white dark:bg-card border border-slate-200 dark:border-border shadow-sm flex items-center justify-center hover:bg-primary hover:text-white hover:border-primary transition-all duration-200 opacity-100"
                  ...
                  >
                    {/* Subject Card */}
                    <div className="bg-white dark:bg-card rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 dark:border-border p-5 md:p-8 shadow-sm hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 h-full flex flex-col group-hover:-translate-y-2 relative overflow-hidden active:scale-[0.98]">
                      ...
                      <h3 className="font-black text-xl md:text-2xl text-slate-800 dark:text-foreground mb-2 md:mb-3 group-hover:text-primary transition-colors leading-tight">
                        {subject.name}
                      </h3>
                      
                      {subject.description && (
                        <p className="text-slate-500 dark:text-muted-foreground text-xs md:text-sm mb-5 md:mb-8 line-clamp-2 leading-relaxed font-medium">
                          {subject.description}
                        </p>
                      )}
                      ...
                      <div className="mt-auto pt-4 md:pt-6 border-t border-slate-50 grid grid-cols-2 gap-y-3 md:gap-y-4 gap-x-2 relative z-10">
                        <div className="flex items-center gap-2 md:gap-3 text-slate-400 dark:text-muted-foreground group-hover:text-slate-600 dark:text-foreground/70 transition-colors">
                          <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-slate-50 dark:bg-muted flex items-center justify-center">
                        ...
                        {subject.author_name && (
                          <div className="col-span-2 flex items-center gap-2 md:gap-3 text-slate-400 dark:text-muted-foreground group-hover:text-slate-600 dark:text-foreground/70 transition-colors">
                            <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-slate-50 dark:bg-muted flex items-center justify-center">
                      ...
                      <div className="flex items-center justify-between mt-5 md:mt-8 text-primary font-black text-[10px] md:text-xs uppercase tracking-[0.15em] md:tracking-[0.2em]">
                        ...
                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-slate-50 dark:bg-muted flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all">
