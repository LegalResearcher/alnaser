              ...
              <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-blue-950 rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-4 fade-in duration-600">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 rounded-2xl bg-white/15 border border-white/20 flex items-center justify-center"><Swords className="w-6 h-6 text-blue-300" /></div>
                  ...
                  {!challengeLink ? (
                    <button onClick={handleCreateChallenge} disabled={isCreatingChallenge}
                      className="w-full h-12 rounded-2xl bg-white/15 border border-white/20 text-white font-black text-sm hover:bg-white/25 transition-all flex items-center justify-center gap-2 active:scale-[0.98]">
                      {isCreatingChallenge ? <><Loader2 className="w-4 h-4 animate-spin" /> جاري الإنشاء...</> : <><Swords className="w-4 h-4" /> إنشاء تحدي بنفس الأسئلة</>}
                    </button>
                  ) : (
                    <div className="space-y-3">
                      <div className="bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-xs text-white/70 font-mono break-all">{challengeLink}</div>
