 ...
 * Version: 23.0 (Smart Universal Parser: Supports Narrative + +/- Patterns + Full CRUD)
 */
...
// --- المحرك الذكي المطور (يدعم كل الأنماط المذكورة) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  // دالة لتحديد هل السطر يمثل خياراً
  ...
  lines.forEach(line => {
    ...
    if (isOptionLine(t)) {
      ...
      // تحديد الإجابة الصحيحة بناءً على علامة (+)
      const isCorrect = t.includes('+');
      
      // تنظيف نص الخيار من (الرقم، القوس، علامة + أو -)
      let cleanVal = t
        .replace(/^[\(\s\d\)\.\-\+]+/, '') // حذف البداية مثل "1) - "
        .replace(/[\+\-]$/, '') // حذف العلامات في النهاية إن وجدت
        .trim();
      ...
      // إذا لم يكن خياراً، فهو سطر تابع للسؤال (سردي)
      
      // إذا وجدنا نصاً جديداً بعد اكتمال سؤال سابق
      ...
      if (!current) {
        current = {
          ...
          question_text: t.replace(/^[\(\d\)\.\-\s]+/, '').trim(), // تنظيف أرقام السؤال في البداية
          option_a: '', option_b: '', option_c: '', option_d: '',
          correct_option: 'A',
          count: 0
        };
      } else {
        // دمج الأسطر المتعددة لنفس السؤال
        current.question_text += ' ' + t.trim();
      }
    }
  });
...
const AdminQuestions = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questions'] });
      toast({ title: role === 'admin' ? 'تم حذف السؤال' : 'تم إرسال طلب الحذف' });
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm"><ScanLine className="w-4 h-4" /> استيراد PDF</Button>
      ...
      <AlertDialog open={!!deleteQuestion} onOpenChange={() => setDeleteQuestion(null)}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          <AlertDialogHeader><AlertDialogTitle className="font-black text-2xl text-slate-900">{role === 'admin' ? 'تأكيد الحذف النهائي' : 'إرسال طلب حذف'}</AlertDialogTitle></AlertDialogHeader>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-cairo font-bold">
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(formData); }} className="space-y-6 mt-4">
            ...
            <div className="mt-4"><Label>ملاحظة (تلميح)</Label><Input value={formData.hint} onChange={(e) => setFormData({...formData, hint: e.target.value})} className="rounded-xl h-12" placeholder="تلميح اختياري" /></div>
            <Button type="submit" disabled={saveMutation.isPending} className="gradient-primary text-white w-full h-12 rounded-xl text-lg font-black shadow-lg">{saveMutation.isPending ? 'جاري الحفظ...' : 'حفظ البيانات'}</Button>
          </form>
        </DialogContent>
      </Dialog>
