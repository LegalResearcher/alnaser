 ...
 * Version: 16.0 (Spatial Parsing + Full CRUD + Bulk Delete)
 */
...
// الرابط كما هو في نسختك المعتمدة
...
const AdminQuestions = () => {
  ...
  // --- 1. دالة الحفظ الجماعي (للاستيراد) ---
  ...
  // --- 2. دالة الحفظ الفردي (إضافة وتعديل) ---
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          ...
          <div className="flex flex-wrap gap-2">
            {/* زر الحذف المتعدد يظهر فقط عند التحديد */}
            ...
            <div className="divide-y divide-slate-50 font-cairo">
              {questions.map((q, i) => (
                <div key={q.id} className={cn("p-6 hover:bg-slate-50 transition-colors group relative flex gap-4 items-start", selectedIds.includes(q.id) && "bg-blue-50/50")}>
                  {/* Checkbox */}
                  <div className="pt-1">
                    <Checkbox checked={selectedIds.includes(q.id)} onCheckedChange={() => toggleSelectOne(q.id)} className="w-5 h-5 border-slate-300 data-[state=checked]:bg-primary" />
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="sm" onClick={() => openEditDialog(q)} className="h-8 w-8 p-0 hover:bg-blue-50 hover:text-blue-600 text-slate-400">
                          <Edit2 className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => setDeleteQuestion(q)} className="h-8 w-8 p-0 hover:bg-red-50 hover:text-red-600 text-slate-400">
            ...
            <div className="space-y-10 pb-12">
              {previewQuestions.filter(x => x.question_text.includes(previewSearch)).map((q) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 relative group/item">
                  ...
                  <Textarea value={q.question_text} onChange={(e) => { const up = [...previewQuestions]; const i = up.findIndex(x => x.id === q.id); up[i].question_text = e.target.value; setPreviewQuestions(up); }} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[100px] font-cairo shadow-sm focus:ring-primary/20" dir="rtl" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {['A', 'B', 'C', 'D'].map(l => (
                      <div key={l} className="space-y-2 font-cairo"><div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black tracking-widest text-slate-500 uppercase">خيار ({l})</Label><button onClick={() => { const up = [...previewQuestions]; const i = up.findIndex(x => x.id === q.id); up[i].correct_option = l; setPreviewQuestions(up); }} className={cn("text-[9px] font-black px-2.5 py-1 rounded-lg transition-all", q.correct_option === l ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400 hover:bg-slate-300")}>{q.correct_option === l ? 'إجابة صحيحة' : 'تحديد'}</button></div><Input value={q[`option_${l.toLowerCase()}`]} onChange={(e) => { const up = [...previewQuestions]; const i = up.findIndex(x => x.id === q.id); up[i][`option_${l.toLowerCase()}`] = e.target.value; setPreviewQuestions(up); }} className="rounded-xl border-slate-200 bg-white font-bold h-12" dir="rtl" /></div>
                    ))}
                  </div>
                </div>
              ))}
