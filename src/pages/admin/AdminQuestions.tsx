...
import { 
  ...
  ChevronUp, ChevronDown, Sparkles, CheckCheck, AlertTriangle
} from 'lucide-react';
...
const AdminQuestions = () => {
  ...
  // --- أداة التصحيح الذكي ---
  const [aiCheckQuestion, setAiCheckQuestion] = useState<Question | null>(null);
  const [aiCheckResult, setAiCheckResult] = useState<{
    hasSuggestions: boolean;
    issues: { field: string; original: string; suggestion: string; reason: string }[];
    summary: string;
  } | null>(null);
  ...
  const runAiCheck = async (q: Question) => {
    setAiCheckQuestion(q);
    setAiCheckResult(null);
    setAiApplied(false);
    setIsAiChecking(true);
    try {
      const prompt = `أنت مدقق لغوي متخصص في اللغة العربية والنصوص القانونية والفقهية. راجع هذا السؤال وخياراته وأبلغ عن أي أخطاء إملائية أو نحوية أو صياغية. إذا كانت الخيارات ج أو د فارغة في سؤال صح/خطأ فهذا طبيعي ولا تعتبره خطأ.
...
أجب بـ JSON فقط بهذا الشكل بدون أي نص خارج الـ JSON:
{
  "hasSuggestions": true أو false,
  "summary": "ملخص قصير جداً",
  "issues": [
    { "field": "question_text أو option_a أو option_b أو option_c أو option_d", "original": "النص الأصلي", "suggestion": "النص المقترح", "reason": "سبب التصحيح" }
  ]
}
إذا لم توجد أخطاء: {"hasSuggestions": false, "summary": "النص سليم ✅", "issues": []}`;
      ...
      setAiCheckResult(parsed);
    } catch (e) {
      toast({ title: 'خطأ في التصحيح الذكي', description: 'حدثت مشكلة أثناء الاتصال بالذكاء الاصطناعي', variant: 'destructive' });
      setAiCheckQuestion(null);
    } finally {
  ...
  const applyAiSuggestions = async () => {
    if (!aiCheckQuestion || !aiCheckResult) return;
    const updated: any = { ...aiCheckQuestion };
    for (const issue of aiCheckResult.issues) {
      if (issue.suggestion) updated[issue.field] = issue.suggestion;
    }
    const payload = {
      ...
      exam_year: updated.exam_year || null,
      exam_form: updated.exam_form || 'General',
    };
    const { error } = await supabase.from('questions').update(payload).eq('id', aiCheckQuestion.id);
    if (error) {
...
<\/script>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        ...
                        {isAdmin && <Button variant="ghost" size="sm" onClick={() => runAiCheck(q)} className="h-8 w-8 p-0 hover:bg-violet-50 hover:text-violet-600 bg-white border border-slate-100" title="تصحيح ذكي"><Sparkles className="w-4 h-4" /></Button>}
      ...
      {/* Dialog التصحيح الذكي */}
      <Dialog open={!!aiCheckQuestion} onOpenChange={(open) => { if (!open) { setAiCheckQuestion(null); setAiCheckResult(null); setAiApplied(false); } }}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-0 z-[9999] font-cairo font-bold shadow-2xl border-none flex flex-col max-h-[90vh]">
          <DialogHeader className="px-8 pt-8 pb-4 border-b border-slate-100 shrink-0">
            <DialogTitle className="text-2xl font-black flex items-center gap-3">
              <Sparkles className="text-violet-500 w-7 h-7" /> التصحيح الذكي للسؤال
            </DialogTitle>
            {aiCheckQuestion && (
              <p className="text-sm text-slate-500 font-bold text-right mt-1 line-clamp-2">{aiCheckQuestion.question_text}</p>
            )}
          </DialogHeader>
          ...
          <ScrollArea className="flex-1 px-8 py-6">
            {isAiChecking && (
              <div className="flex flex-col items-center justify-center py-16 gap-4">
                ...
                <p className="text-slate-500 font-black">جارٍ تحليل السؤال بالذكاء الاصطناعي...</p>
              </div>
            )}
            ...
            {!isAiChecking && aiCheckResult && (
              <div className="space-y-4">
                ...
                <div className={cn(
                  ...
                  aiCheckResult.hasSuggestions
                    ? "bg-amber-50 border-amber-200 text-amber-800"
                    : "bg-emerald-50 border-emerald-200 text-emerald-800"
                )}>
                  {aiCheckResult.hasSuggestions
                    ? <AlertTriangle className="w-5 h-5 shrink-0 text-amber-500" />
                    : <CheckCheck className="w-5 h-5 shrink-0 text-emerald-500" />
                  }
                  <span className="font-black text-sm">{aiCheckResult.summary}</span>
                </div>
                      ...
                      return (
                        <div key={idx} className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden">
                          <div className="bg-slate-100 px-4 py-2 flex items-center justify-between">
                            <span className="text-xs font-black text-slate-600">{fieldLabels[issue.field] || issue.field}</span>
                            <span className="text-xs text-slate-400 font-bold">{issue.reason}</span>
                          </div>
                          <div className="p-4 space-y-2">
                            <div className="flex items-start gap-2">
                              <span className="text-xs font-black text-red-500 shrink-0 mt-0.5">قبل:</span>
                              <span className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-1.5 flex-1 text-right leading-relaxed">{issue.original}</span>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-xs font-black text-emerald-500 shrink-0 mt-0.5">بعد:</span>
                              <span className="text-sm text-emerald-700 bg-emerald-50 rounded-lg px-3 py-1.5 flex-1 text-right leading-relaxed font-bold">{issue.suggestion}</span>
                            </div>
                          </div>
                        </div>
                      );
          ...
          {!isAiChecking && aiCheckResult && (
            <div className="sticky bottom-0 bg-white border-t px-8 py-5 rounded-b-3xl flex gap-3">
              <Button
                variant="outline"
                onClick={() => { setAiCheckQuestion(null); setAiCheckResult(null); setAiApplied(false); }}
                className="flex-1 rounded-xl font-black h-12"
              >
                {aiApplied ? 'إغلاق' : 'تجاهل'}
              </Button>
              {aiCheckResult.hasSuggestions && !aiApplied && (
                <Button
                  onClick={applyAiSuggestions}
                  className="flex-1 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-black h-12 gap-2"
                >
                  <CheckCheck className="w-4 h-4" /> تطبيق التصحيحات
                </Button>
              )}
              {aiApplied && (
                <div className="flex-1 flex items-center justify-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 font-black text-sm">
                  <CheckCheck className="w-4 h-4" /> تم التطبيق بنجاح ✅
                </div>
              )}
            </div>
          )}
