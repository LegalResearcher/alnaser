 ...
 * Version: 13.0 (Bulk Save Fix - Matches Original Logic)
 */
...
// --- المحرك الذكي المطور (Spatial Spatial Parsing) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // التعرف على الخيارات (تبدأ بـ رقم أو علامة +)
...
const AdminQuestions = () => {
  ...
  // --- دالة حفظ الكل (تم تعديلها لتطابق منطق كودك الأصلي الناجح) ---
  const bulkSaveMutation = useMutation({
    mutationFn: async (list: any[]) => {
      if (!selectedSubject) throw new Error("يرجى اختيار المادة التعليمية أولاً");

      // تجهيز البيانات بنفس حقول الكود الأصلي
      const dataToInsert = list.map(q => ({
        ...
        option_c: q.option_c || '',
        option_d: q.option_d || '',
        correct_option: q.correct_option, // الحقل في قاعدة بياناتك هو correct_option
        hint: null,
        exam_year: selectedYear ? parseInt(selectedYear) : null,
        created_by: user?.id,
      }));

      // استخدام نفس دالة الإدخال الأصلية
    ...
    onSuccess: () => {
      ...
      toast({ title: 'تمت إضافة الأسئلة بنجاح! ✅', description: `تم حفظ ${previewQuestions.length} سؤال.` });
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'تنبيه', description: 'لم نتمكن من تحليل الملف، تأكد من جودته.', variant: 'destructive' });
      }
    } catch (err) {
      toast({ title: 'خطأ فني', description: 'تأكد من تثبيت مكتبة PDF المذكورة سابقاً.', variant: 'destructive' });
    } finally {
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام الاستيراد الذكي لجامعة صنعاء</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm">
              <ScanLine className="w-4 h-4" /> استيراد نماذج PDF
            </Button>
            <Button size="sm" className="gradient-primary text-white gap-2 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20 font-bold font-cairo">
          <Select value={selectedLevel} onValueChange={setSelectedLevel}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="المستوى" /></SelectTrigger><SelectContent className="z-[9999] bg-white">{levels.map(l => <SelectItem key={l.id} value={l.id}>{l.name}</SelectItem>)}</SelectContent></Select>
          <Select value={selectedSubject} onValueChange={setSelectedSubject}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="المادة" /></SelectTrigger><SelectContent className="z-[9999] bg-white">{subjects.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select>
          <Select value={selectedYear || "all"} onValueChange={(v) => setSelectedYear(v === "all" ? "" : v)}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="السنة" /></SelectTrigger><SelectContent className="z-[9999] bg-white shadow-2xl"><SelectItem value="all">كافة السنوات</SelectItem>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}</SelectContent></Select>
          ...
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            ...
            <div className="flex gap-3">
               ...
               <Button 
                ...
                className="gradient-primary text-white font-black px-12 rounded-xl shadow-xl"
               >
            ...
            <div className="space-y-10 pb-12">
              {previewQuestions.filter(x => x.question_text.includes(previewSearch)).map((q) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 relative group/item">
                  ...
                  }} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[90px] font-cairo shadow-sm" dir="rtl" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {['A', 'B', 'C', 'D'].map(l => (
                      <div key={l} className="space-y-2">
                        <Label className="text-[10px] font-black font-cairo">الخيار ({l})</Label>
      ...
      <Dialog open={isFileUploadOpen} onOpenChange={setIsFileUploadOpen}>
        <DialogContent className="max-w-md bg-white rounded-3xl p-10 shadow-2xl border-none z-[9999] font-cairo">
          <DialogHeader><DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900 font-cairo"><ScanLine className="text-primary" /> رفع ملف الاختبار</DialogTitle></DialogHeader>
          <div onClick={() => fileInputRef.current?.click()} className="mt-8 border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center hover:border-primary/40 hover:bg-primary/5 cursor-pointer group transition-all relative overflow-hidden">
            ...
            {isProcessingFile ? (
              <div className="flex flex-col items-center gap-4"><Loader2 className="w-12 h-12 text-primary animate-spin" /><p className="text-sm font-black text-slate-600">جاري المعالجة الذكية...</p></div>
            ) : (
              <div className="flex flex-col items-center gap-4"><div className="p-5 bg-slate-100 rounded-2xl transition-all shadow-sm group-hover:bg-white"><FileText className="w-10 h-10 text-slate-400 group-hover:text-primary" /></div><p className="font-black text-slate-700 text-lg font-cairo">اضغط لاختيار ملف PDF</p></div>
            )}
          </div>
        </DialogContent>
      </Dialog>
