 ...
 * Version: 12.0 (Final Master Build - Spatial Parsing & Bulk Save Fix)
 */
...
// --- المحرك الذكي المطور (Spatial Pattern Parsing) لمنع تداخل النصوص ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // التعرف على الخيارات (تبدأ برقم أو علامة +)
...
const AdminQuestions = () => {
  ...
  // --- دالة حفظ الكل مع معالجة الأخطاء والتحقق من المادة ---
  const bulkSaveMutation = useMutation({
    mutationFn: async (list: any[]) => {
      if (!selectedSubject) {
        throw new Error("يرجى اختيار المادة التعليمية أولاً من القائمة العلوية");
      }
    ...
    onSuccess: () => {
      ...
      toast({ title: 'تم الحفظ بنجاح! ✅', description: `تمت إضافة ${previewQuestions.length} سؤال إلى بنك الأسئلة.` });
    ...
    onError: (err: any) => {
      toast({ 
        title: 'فشل الحفظ ❌', 
        description: `السبب: ${err.message}`, 
        variant: 'destructive' 
      });
    }
  });

  // --- معالجة الـ PDF بترتيب الإحداثيات (Y ثم X) لمنع التداخل البصري ---
  const processPDF = async (file: File) => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      if (textContent.items.length > 0) {
        ...
        // ترتيب مكاني دقيق: الأعلى للأسفل، اليمين لليسار (للغة العربية)
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'تنبيه', description: 'لم يتم العثور على أسئلة، تأكد من تنسيق الملف.', variant: 'destructive' });
      }
    } catch (err) {
      toast({ title: 'خطأ', description: 'حدثت مشكلة أثناء المعالجة الذكية.', variant: 'destructive' });
    } finally {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6 font-cairo">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام "الناصر تِك" للاستيراد الدقيق (PDF/OCR)</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm">
              <ScanLine className="w-4 h-4" /> استيراد PDF (بدون تداخل)
            </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20 font-bold">
          ...
          <Select value={selectedYear || "all"} onValueChange={(v) => setSelectedYear(v === "all" ? "" : v)}><SelectTrigger className="bg-white border-slate-200 rounded-xl"><SelectValue placeholder="السنة" /></SelectTrigger><SelectContent className="z-[9999] bg-white shadow-2xl"><SelectItem value="all">كافة السنوات</SelectItem>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}</SelectContent></Select>
      ...
      {/* --- حوار المعاينة المطور (تصحيح التداخل البصري والوظيفي) --- */}
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-5xl bg-white rounded-3xl p-8 border-none shadow-2xl h-[94vh] flex flex-col overflow-hidden z-[9999]">
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            <div><DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900 font-black"><Eye className="text-primary w-8 h-8" /> مراجعة استخراج الـ PDF</DialogTitle><p className="text-xs text-slate-400 font-bold mt-1">لديك ({previewQuestions.length}) سؤال جاهز للحفظ.</p></div>
            <div className="flex gap-3">
               <Button variant="ghost" onClick={() => setIsPreviewOpen(false)} className="rounded-xl font-bold px-8 underline">إلغاء</Button>
               <Button 
                onClick={() => bulkSaveMutation.mutate(previewQuestions)} 
                disabled={bulkSaveMutation.isPending || previewQuestions.length === 0} 
                className="gradient-primary text-white font-black px-12 rounded-xl shadow-xl transition-all hover:scale-105 active:scale-95"
               >
                 {bulkSaveMutation.isPending ? <Loader2 className="animate-spin ml-2" /> : <Save className="ml-2 w-4 h-4" />} 
                 {bulkSaveMutation.isPending ? "جاري الحفظ..." : "حفظ الكل في القاعدة"}
               </Button>
            </div>
          </DialogHeader>
          <div className="mt-4"><Input placeholder="بحث داخل النتائج لتعديلها..." value={previewSearch} onChange={(e) => setPreviewSearch(e.target.value)} className="h-12 bg-slate-50 border-slate-100 rounded-2xl font-bold" /></div>
          <ScrollArea className="flex-1 mt-6 pr-4">
            <div className="space-y-10 pb-12">
              {previewQuestions.filter(x => x.question_text.includes(previewSearch)).map((q) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 relative group/item hover:bg-white transition-all">
                  <button onClick={() => setPreviewQuestions(prev => prev.filter(p => p.id !== q.id))} className="absolute -left-3 -top-3 w-10 h-10 bg-white border border-red-100 text-destructive rounded-2xl flex items-center justify-center shadow-lg hover:bg-destructive hover:text-white z-30 opacity-0 group-hover/item:opacity-100 transition-all"><Trash2 className="w-5 h-5" /></button>
                  ...
                  }} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[100px] shadow-sm focus:ring-primary/20" dir="rtl" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {['A', 'B', 'C', 'D'].map(letter => (
                      <div key={letter} className="space-y-2 font-bold">
                        <div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black text-slate-500 uppercase">خيار ({letter})</Label><button onClick={() => {
                            const up = [...previewQuestions];
                            const i = up.findIndex(x => x.id === q.id);
                            up[i].correct_option = letter;
                            setPreviewQuestions(up);
                        }} className={cn("text-[9px] font-black px-3 py-1.5 rounded-lg transition-all shadow-sm", q.correct_option === letter ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400 hover:bg-slate-200")}>{q.correct_option === letter ? 'إجابة صحيحة ✅' : 'تحديد كإجابة'}</button></div>
                        <Input value={q[`option_${letter.toLowerCase()}`]} onChange={(e) => {
                           const up = [...previewQuestions];
                           const i = up.findIndex(x => x.id === q.id);
                           up[i][`option_${letter.toLowerCase()}`] = e.target.value;
                           setPreviewQuestions(up);
                        }} className="rounded-xl border-slate-200 bg-white font-bold h-12 shadow-inner" dir="rtl" />
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <Dialog open={isFileUploadOpen} onOpenChange={setIsFileUploadOpen}>
        <DialogContent className="max-w-md bg-white rounded-3xl p-10 shadow-2xl border-none z-[9999]">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="mt-8 border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center hover:border-primary/40 hover:bg-primary/5 cursor-pointer group transition-all relative overflow-hidden">
            ...
            {isProcessingFile ? (
              <div className="flex flex-col items-center gap-4"><Loader2 className="w-12 h-12 text-primary animate-spin" /><p className="text-sm font-black text-slate-600 text-center">جاري استخراج الأسئلة مكانياً...<br/>يرجى الانتظار</p></div>
            ) : (
      ...
      {/* حوارات الإضافة والحذف الاعتيادية */}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-bold border-none shadow-2xl">
          <DialogHeader><DialogTitle className="text-2xl font-black tracking-tight">إضافة سؤال جديد</DialogTitle></DialogHeader>
          <form onSubmit={(e) => { e.preventDefault(); bulkSaveMutation.mutate([formData]); setIsDialogOpen(false); }} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {['a', 'b', 'c', 'd'].map(l => <Input key={l} placeholder={`الخيار (${l.toUpperCase()})`} value={formData[`option_${l}` as keyof typeof formData]} onChange={(e) => setFormData({ ...formData, [`option_${l}`]: e.target.value })} className="rounded-xl h-12" />)}
            </div>
            <Button type="submit" disabled={bulkSaveMutation.isPending} className="gradient-primary text-white w-full h-12 rounded-xl text-lg font-black shadow-lg">حفظ البيانات</Button>
          </form>
        </DialogContent>
      </Dialog>
    </AdminLayout>
