 ...
 * Version: 20.0 (Smart Universal Parser + MODIFIED Delete Logic from Old Code)
 */
...
const AdminQuestions = () => {
  // جلب role من useAuth لتطبيق منطق الحذف القديم
  ...
  // =================================================================
  // === تعديل منطق الحذف بناءً على الكود القديم ليتناسب مع الصلاحيات ===
  // =================================================================
  const deleteMutation = useMutation({
    mutationFn: async (question: Question) => {
      if (role === 'admin') {
        // إذا كان مسؤولاً: حذف نهائي
        const { error } = await supabase.from('questions').delete().eq('id', question.id);
        if (error) throw error;
      } else {
        // إذا لم يكن مسؤولاً: طلب حذف وتغيير الحالة إلى pending_deletion
        const { error: requestError } = await supabase.from('deletion_requests').insert({
          question_id: question.id,
          requested_by: user?.id,
        });
        if (requestError) throw requestError;

        const { error: statusError } = await supabase.from('questions').update({ status: 'pending_deletion' }).eq('id', question.id);
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (role === 'admin') {
        ...
        // طلب حذف متعدد للمستخدمين الآخرين
        const requests = selectedIds.map(id => ({ question_id: id, requested_by: user?.id }));
        const { error: requestError } = await supabase.from('deletion_requests').insert(requests);
        if (requestError) throw requestError;

        const { error: statusError } = await supabase.from('questions').update({ status: 'pending_deletion' }).in('id', selectedIds);
        if (statusError) throw statusError;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questions'] });
      toast({ title: role === 'admin' ? `تم حذف ${selectedIds.length} سؤال` : `تم إرسال طلبات حذف لـ ${selectedIds.length} سؤال` });
    ...
    <AdminLayout>
      ...
      <AlertDialog open={!!deleteQuestion} onOpenChange={() => setDeleteQuestion(null)}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          ...
          <AlertDialogFooter className="gap-3 mt-8 font-bold">
            ...
            >
              {role === 'admin' ? 'حذف نهائي' : 'إرسال الطلب'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-black text-2xl text-slate-900">
              حذف ({selectedIds.length}) سؤال
            </AlertDialogTitle>
            <AlertDialogDescription className="font-medium text-slate-600">
              {role === 'admin' 
                ? 'سيتم حذف جميع الأسئلة المحددة نهائياً. هل أنت متأكد؟' 
                : 'سيتم إرسال طلبات حذف للأسئلة المحددة للمسؤول.'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-3 mt-8 font-bold">
            <AlertDialogCancel className="rounded-xl font-bold">تراجع</AlertDialogCancel>
            <AlertDialogAction 
              ...
              className="bg-destructive text-white rounded-xl font-black px-10"
            >
              {bulkDeleteMutation.isPending ? 'جاري التنفيذ...' : 'تأكيد الحذف'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
