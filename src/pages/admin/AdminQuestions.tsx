 ...
 * Version: 11.0 (Advanced Spatial Parsing - Anti-Overlap Fix)
 * Fix: Bulk Save Logic Only
 */
...
// الرابط كما طلبته بالضبط (بدون تعديل)
pdfjsLib.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsLib.version}/pdf.worker.min.js`;

// --- المحرك الذكي المطور (Spatial Pattern Parsing) ---
// لم يتم تعديل أي شيء هنا بناءً على طلبك
...
const AdminQuestions = () => {
  ...
  // --- دالة الحفظ الجماعي (هنا فقط التعديل لإصلاح الحفظ) ---
  const bulkSave = useMutation({
    mutationFn: async (list: any[]) => {
      // التحقق من الشروط الضرورية لقاعدة البيانات
      if (!selectedSubject) throw new Error("يرجى اختيار المادة التعليمية أولاً");
      if (!user?.id) throw new Error("يرجى التأكد من تسجيل الدخول");
      ...
      const formatted = list.map(q => ({
        ...
        created_by: user.id, // إضافة معرف المستخدم ضرورية للـ RLS
        status: 'active'
      }));

      // استخدام .select() للتأكد من إرجاع البيانات ومعرفة الخطأ إن وجد
    ...
    onSuccess: () => {
      ...
      toast({ title: 'تم الحفظ بنجاح! ✅', description: `تمت إضافة ${previewQuestions.length} سؤال للقاعدة.` });
    ...
    onError: (err: any) => {
      toast({ 
        title: 'فشل الحفظ', 
        description: err.message || "تأكد من اختيار المادة وصلاحيات المستخدم", 
        variant: 'destructive' 
      });
    }
  });
