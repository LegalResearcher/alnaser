 ...
 * Version: 16.0 (Full CRUD + Bulk Delete Feature)
 */
...
import { 
  ...
  Eye, Save, X, PencilLine, ScanLine, CheckSquare, Square
} from 'lucide-react';
...
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    const questionMatch = t.match(/^\(?\d+\)?\s*(.*?):$/) || t.includes('؟');
    
    if (questionMatch || (t.endsWith(':') && !t.match(/^\(?\d+\)?\s*-/))) {
    ...
    else if (t.match(/^\(?\d+\)?\s*[-+]/) || t.match(/^\d+\s*\)/)) {
      if (!current) return;
      const isCorrect = t.includes('+');
      let cleanVal = t.replace(/^\(?\d+\)?\s*[-+]\s*/, '').replace(/^\d+\s*\)\s*[-+]\s*/, '').trim();
...
const AdminQuestions = () => {
  ...
  const [deleteQuestion, setDeleteQuestion] = useState<Question | null>(null); // للحذف الفردي
  
  // --- حالات التحديد المتعدد (جديد) ---
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'تنبيه', description: 'لم نتمكن من فرز الأسئلة.', variant: 'destructive' });
      }
    } catch (err) {
      toast({ title: 'خطأ', description: 'حدثت مشكلة تقنية.', variant: 'destructive' });
    } finally {
  ...
  // --- دالة الحفظ الجماعي (الاستيراد) ---
  const bulkSave = useMutation({
    mutationFn: async (list: any[]) => {
      if (!selectedSubject) throw new Error("يجب اختيار المادة أولاً");
    ...
    onError: (err: any) => toast({ title: '❌ فشل الاستيراد', description: err.message, variant: 'destructive' })
  });

  // --- دالة الحفظ الفردي (إضافة/تعديل) ---
  ...
  // --- دالة الحذف المتعدد (جديد) ---
  const bulkDeleteMutation = useMutation({
    mutationFn: async () => {
      if (selectedIds.length === 0) return;
      const { error } = await supabase
        .from('questions')
        .update({ status: 'deleted' }) // حذف ناعم (Soft Delete)
        .in('id', selectedIds);
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          ...
          <div className="flex flex-wrap gap-2">
            {/* أزرار الإجراءات الجماعية (تظهر عند التحديد) */}
            {selectedIds.length > 0 && (
              <Button variant="destructive" size="sm" onClick={() => setIsBulkDeleteDialogOpen(true)} className="gap-2 shadow-lg animate-in fade-in zoom-in duration-300">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden z-10 min-h-[400px]">
          {!selectedSubject ? (
            ...
            <div className="divide-y divide-slate-50 font-cairo">
              {questions.map((q, i) => (
                <div key={q.id} className={cn("p-6 hover:bg-slate-50 transition-colors group relative flex gap-4", selectedIds.includes(q.id) && "bg-blue-50/50")}>
                  {/* مربع الاختيار لكل سؤال */}
                  <div className="pt-1">
                    <Checkbox 
                  ...
                  <div className="flex-1">
                    <div className="absolute top-6 left-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                      <Button variant="outline" size="sm" onClick={() => { setEditingQuestion(q); setFormData({ question_text: q.question_text, option_a: q.option_a, option_b: q.option_b, option_c: q.option_c || '', option_d: q.option_d || '', correct_option: q.correct_option as any, hint: q.hint || '', exam_year: q.exam_year?.toString() || '' }); setIsDialogOpen(true); }} className="h-8 w-8 p-0 rounded-lg border-slate-200 hover:bg-blue-50 hover:text-blue-600 bg-white shadow-sm">
                        <Edit2 className="w-4 h-4" />
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => setDeleteQuestion(q)} className="h-8 w-8 p-0 rounded-lg border-slate-200 hover:bg-red-50 hover:text-red-600 bg-white shadow-sm">
      ...
      {/* مودالات الاستيراد والرفع (كما هي) */}
            ...
            <div className="space-y-10 pb-12">
              {previewQuestions.filter(x => x.question_text.includes(previewSearch)).map((q) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 relative group/item">
                  ...
                  }} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[100px] font-cairo shadow-sm focus:ring-primary/20" dir="rtl" />
                    ...
                    {['A', 'B', 'C', 'D'].map(letter => (
                      <div key={letter} className="space-y-2 font-cairo">
                        <div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black tracking-widest text-slate-500 uppercase">خيار ({letter})</Label><button onClick={() => {
                            ...
                            setPreviewQuestions(up);
                        }} className={cn("text-[9px] font-black px-2.5 py-1 rounded-lg transition-all", q.correct_option === letter ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400 hover:bg-slate-300")}>{q.correct_option === letter ? 'إجابة صحيحة' : 'تحديد'}</button></div>
      ...
      {/* حوار الحذف المتعدد (جديد) */}
      <AlertDialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          <AlertDialogHeader><AlertDialogTitle className="font-black text-2xl text-slate-900 text-destructive">حذف {selectedIds.length} سؤال</AlertDialogTitle></AlertDialogHeader>
          <AlertDialogDescription className="font-bold text-slate-600">هل أنت متأكد من حذف جميع الأسئلة المحددة؟ لا يمكن التراجع عن هذا الإجراء.</AlertDialogDescription>
          <AlertDialogFooter className="gap-3 mt-8 font-bold font-cairo"><AlertDialogCancel className="rounded-xl font-bold">تراجع</AlertDialogCancel><AlertDialogAction onClick={() => bulkDeleteMutation.mutate()} disabled={bulkDeleteMutation.isPending} className="bg-destructive text-white rounded-xl font-black px-10">{bulkDeleteMutation.isPending ? 'جاري الحذف...' : 'حذف الجميع'}</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-cairo font-bold">
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveSingleMutation.mutate(formData); }} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {['a', 'b', 'c', 'd'].map(l => <div key={l}><Label className="text-xs mb-1 block">خيار {l.toUpperCase()}</Label><Input placeholder={`نص الخيار...`} value={formData[`option_${l}` as keyof typeof formData]} onChange={(e) => setFormData({ ...formData, [`option_${l}`]: e.target.value })} className="rounded-xl h-12" /></div>)}
            </div>
            <div className="grid grid-cols-2 gap-4">
               ...
               <div><Label>السنة (اختياري)</Label><Select value={formData.exam_year} onValueChange={(v) => setFormData({...formData, exam_year: v})}><SelectTrigger className="rounded-xl h-12"><SelectValue placeholder="اختر السنة" /></SelectTrigger><SelectContent>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}</SelectContent></Select></div>
            </div>
