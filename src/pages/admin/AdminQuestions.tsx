 ...
 * Fix: Removed SelectPortal and used direct Z-index for stable build
 * Version: Ultimate Integrated Build (PDF + OCR + Smart Parser + Bulk Save)
 */
...
import { 
  ...
  Eye, Save, X, PencilLine, ScanLine 
} from 'lucide-react';
...
// --- المحرك الذكي المطور (نمط جامعة صنعاء القانوني) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // اكتشاف السؤال: يبدأ برقم وينتهي بـ (:) أو (؟)
    if (t.match(/^\(?\d+\)?\s*(.*?):$/) || t.includes('؟') || (t.endsWith(':') && !t.includes('-'))) {
      if (current && current.option_a) results.push(current);
      current = {
        id: Math.random().toString(36).substr(2, 9),
        question_text: t.replace(/^\(?\d+\)?\s*/, '').replace(/\.+$/, '').trim(),
        option_a: '', option_b: '', option_c: '', option_d: '',
        correct_option: 'A',
        count: 0
      };
    } 
    // اكتشاف الخيارات: نمط (رقم) أو علامة +
...
const AdminQuestions = () => {
  ...
  // --- منطق الحفظ الجماعي (Bulk Save) المصحح ---
  const bulkSaveMutation = useMutation({
    mutationFn: async (list: any[]) => {
      if (!selectedSubject) throw new Error("يرجى اختيار المادة التعليمية");
    ...
    onError: (err: any) => {
      toast({ title: 'فشل الحفظ في قاعدة البيانات', description: err.message, variant: 'destructive' });
    }
  });

  // --- دالة معالجة الـ PDF (الترتيب المكاني) ---
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'فشل التحليل', description: 'تأكد من جودة الملف ونمط الكتابة.', variant: 'destructive' });
      }
    } catch (err) {
      toast({ title: 'خطأ تقني', description: 'حدثت مشكلة في معالجة الملف بصرياً', variant: 'destructive' });
    } finally {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questions'] });
      toast({ title: editingQuestion ? 'تم التحديث' : 'تمت الإضافة بنجاح' });
      handleCloseDialog();
    }
  });
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام الناصر تِك المتطور للاستيراد والحفظ</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm">
              <ScanLine className="w-4 h-4" /> استيراد PDF / OCR
            </Button>
        ...
        {/* Filters */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20 font-bold font-cairo text-xs">
          <Select value={selectedLevel} onValueChange={setSelectedLevel}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="اختر المستوى" /></SelectTrigger><SelectContent className="z-[9999] bg-white">{levels.map(l => <SelectItem key={l.id} value={l.id}>{l.name}</SelectItem>)}</SelectContent></Select>
          <Select value={selectedSubject} onValueChange={setSelectedSubject}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="اختر المادة" /></SelectTrigger><SelectContent className="z-[9999] bg-white">{subjects.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select>
          <Select value={selectedYear || "all"} onValueChange={(v) => setSelectedYear(v === "all" ? "" : v)}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="سنة النموذج" /></SelectTrigger><SelectContent className="z-[9999] bg-white shadow-2xl overflow-y-auto"><SelectItem value="all">كافة السنوات</SelectItem>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>نموذج {y}</SelectItem>)}</SelectContent></Select>
          <div className="relative"><Search className="absolute right-3 top-2.5 w-4 h-4 text-slate-400" /><Input placeholder="بحث نصي..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pr-10 bg-white border-slate-200 rounded-xl" /></div>
        </div>
                ...
                <div key={q.id} className="p-6 hover:bg-slate-50 transition-colors group">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                       <div className="flex items-center gap-2 mb-2"><span className="w-6 h-6 rounded-lg bg-primary/10 text-primary text-[10px] font-black flex items-center justify-center">{i + 1}</span>{q.exam_year && <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">نموذج {q.exam_year}</span>}</div>
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><Button variant="ghost" size="icon" onClick={() => { setEditingQuestion(q); setFormData({ question_text: q.question_text, option_a: q.option_a, option_b: q.option_b, option_c: q.option_c || '', option_d: q.option_d || '', correct_option: q.correct_option, hint: q.hint || '', exam_year: q.exam_year?.toString() || '' }); setIsDialogOpen(true); }}><Edit2 className="w-4 h-4" /></Button><Button variant="ghost" size="icon" className="text-destructive" onClick={() => setDeleteQuestion(q)}><Trash2 className="w-4 h-4" /></Button></div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-5xl bg-white rounded-3xl p-8 border-none shadow-2xl h-[94vh] flex flex-col overflow-hidden z-[9999] font-cairo">
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            <DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900 font-black"><Eye className="text-primary w-8 h-8" /> مراجعة استخراج الـ PDF</DialogTitle>
            <div className="flex gap-3">
               <Button variant="ghost" onClick={() => setIsPreviewOpen(false)} className="rounded-xl font-bold px-8 underline">إلغاء</Button>
               <Button onClick={() => bulkSaveMutation.mutate(previewQuestions)} disabled={bulkSaveMutation.isPending || previewQuestions.length === 0} className="gradient-primary text-white font-black px-12 rounded-xl shadow-xl">
                 {bulkSaveMutation.isPending ? <Loader2 className="animate-spin ml-2" /> : <Save className="ml-2 w-4 h-4" />} حفظ الكل
               </Button>
            </div>
          </DialogHeader>
          ...
          <ScrollArea className="flex-1 mt-6 pr-4">
            <div className="space-y-10 pb-12">
              {previewQuestions.filter(x => x.question_text.includes(previewSearch)).map((q) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 relative group/item hover:bg-white transition-all shadow-sm">
                  ...
                  <Textarea value={q.question_text} onChange={(e) => {
                    const up = [...previewQuestions]; const i = up.findIndex(x => x.id === q.id); up[i].question_text = e.target.value; setPreviewQuestions(up);
                  }} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[100px] font-cairo shadow-inner" dir="rtl" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {['A', 'B', 'C', 'D'].map(letter => (
                      <div key={letter} className="space-y-2 font-cairo bg-white p-4 rounded-2xl border border-slate-50">
                        <div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black tracking-widest text-slate-500 uppercase">خيار ({letter})</Label><button onClick={() => {
                            const up = [...previewQuestions]; const i = up.findIndex(x => x.id === q.id); up[i].correct_option = letter; setPreviewQuestions(up);
                        }} className={cn("text-[9px] font-black px-2.5 py-1.5 rounded-lg transition-all", q.correct_option === letter ? "bg-emerald-500 text-white shadow-lg" : "bg-slate-100 text-slate-400")}>{q.correct_option === letter ? 'إجابة صحيحة ✅' : 'تحديد كإجابة'}</button></div>
                        <Input value={q[`option_${letter.toLowerCase()}`]} onChange={(e) => {
                           const up = [...previewQuestions]; const i = up.findIndex(x => x.id === q.id); up[i][`option_${letter.toLowerCase()}`] = e.target.value; setPreviewQuestions(up);
      ...
      <Dialog open={isFileUploadOpen} onOpenChange={setIsFileUploadOpen}>
        <DialogContent className="max-w-md bg-white rounded-3xl p-10 shadow-2xl border-none z-[9999] font-cairo">
          <DialogHeader><DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900"><ScanLine className="text-primary" /> مستورد "الناصر تِك" الذكي</DialogTitle></DialogHeader>
          <div onClick={() => fileInputRef.current?.click()} className="mt-8 border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center hover:border-primary/40 hover:bg-primary/5 cursor-pointer group transition-all relative overflow-hidden">
            ...
            {isProcessingFile ? (
              <div className="flex flex-col items-center gap-4"><Loader2 className="w-12 h-12 text-primary animate-spin" /><p className="text-sm font-black text-slate-600">جاري مسح ومعالجة الملف...</p></div>
            ) : (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-cairo font-bold border-none shadow-2xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(formData); }} className="space-y-6 mt-4">
            <div className="space-y-2.5"><Label className="font-black text-slate-700">نص السؤال *</Label><Textarea value={formData.question_text} onChange={(e) => setFormData({ ...formData, question_text: e.target.value })} required className="rounded-2xl border-slate-200 min-h-[120px] shadow-sm" /></div>
            <div className="grid grid-cols-2 gap-4">
              {['a', 'b', 'c', 'd'].map(l => <Input key={l} placeholder={`خيار (${l.toUpperCase()})`} value={formData[`option_${l}` as keyof typeof formData]} onChange={(e) => setFormData({ ...formData, [`option_${l}`]: e.target.value })} className="rounded-xl h-12 shadow-sm" />)}
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Select value={formData.correct_option} onValueChange={(v) => setFormData({ ...formData, correct_option: v as 'A'|'B'|'C'|'D' })}>
                <SelectTrigger className="rounded-xl h-12"><SelectValue placeholder="الإجابة الصحيحة" /></SelectTrigger>
              ...
              <Select value={formData.exam_year} onValueChange={(v) => setFormData({ ...formData, exam_year: v })}>
                <SelectTrigger className="rounded-xl h-12"><SelectValue placeholder="السنة" /></SelectTrigger>
                <SelectContent className="bg-white z-[9999] shadow-xl">{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <Button type="submit" disabled={saveMutation.isPending} className="gradient-primary text-white w-full h-12 rounded-xl text-lg font-black shadow-lg">حفظ البيانات</Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteQuestion} onOpenChange={() => setDeleteQuestion(null)}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl">
          <AlertDialogHeader><AlertDialogTitle className="font-black text-2xl text-slate-900 font-cairo">تأكيد عملية الحذف</AlertDialogTitle></AlertDialogHeader>
          <AlertDialogFooter className="gap-3 mt-8 font-bold font-cairo"><AlertDialogCancel className="rounded-xl font-bold">تراجع</AlertDialogCancel><AlertDialogAction onClick={() => deleteQuestion && supabase.from('questions').update({ status: 'deleted' }).eq('id', deleteQuestion.id).then(() => queryClient.invalidateQueries())} className="bg-destructive text-white rounded-xl font-black px-10 shadow-lg">حذف نهائي</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
