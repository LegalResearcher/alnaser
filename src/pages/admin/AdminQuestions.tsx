 ...
 * Version: 11.5 (Advanced Spatial Parsing + FIXED Bulk Save Only)
 */
...
// --- المحرك الذكي المطور (كما طلبت تماماً بدون تغيير) ---
...
const AdminQuestions = () => {
  ...
  // --- دالة المعالجة (كما طلبت تماماً بدون تغيير) ---
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      if (textContent.items.length > 0) {
        ...
        items.sort((a, b) => {
          ...
          if (Math.abs(yDiff) < 5) return b.transform[4] - a.transform[4];
          return yDiff;
        });
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'تنبيه', description: 'لم نتمكن من فرز الأسئلة، تأكد من تنسيق النموذج.', variant: 'destructive' });
      }
    } catch (err) {
  ...
  // ===============================================
  // === تم تعديل دالة الحفظ فقط لضمان عملها ===
  // ===============================================
  const bulkSave = useMutation({
    mutationFn: async (list: any[]) => {
      // 1. التحقق من البيانات الإلزامية قبل الإرسال
      if (!selectedSubject) throw new Error("يجب اختيار المادة الدراسية من القائمة أولاً!");
      if (!user?.id) throw new Error("يجب تسجيل الدخول لإتمام عملية الحفظ");

      // 2. تجهيز البيانات بشكل صحيح لقاعدة البيانات
      const formatted = list.map(q => ({
        subject_id: selectedSubject,
        question_text: q.question_text || "سؤال بدون نص",
        option_a: q.option_a || "...",
        option_b: q.option_b || "...",
        option_c: q.option_c || null, // Supabase يفضل null للحقول الفارغة
        option_d: q.option_d || null,
        correct_option: q.correct_option || 'A',
        exam_year: selectedYear ? parseInt(selectedYear) : null,
        created_by: user.id, // ضروري جداً
        status: 'active'
      }));

      // 3. الإرسال مع التحقق من الخطأ
      const { data, error } = await supabase
        .from('questions')
        .insert(formatted)
        .select(); // إضافة select للتأكد من نجاح العملية
    ...
    onSuccess: () => {
      ...
      toast({ 
        ...
        description: `تمت إضافة ${previewQuestions.length} سؤال لقاعدة البيانات.` 
      });
    ...
    onError: (err: any) => {
      toast({ 
        title: '❌ خطأ في الحفظ', 
        description: err.message || "تأكد من اختيار المادة وصلاحياتك", 
        variant: 'destructive' 
      });
    }
  });
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام "الناصر تِك" للاستيراد المكاني الدقيق</p>
          </div>
        ...
        <DialogContent className="max-w-5xl bg-white rounded-3xl p-8 border-none shadow-2xl h-[94vh] flex flex-col overflow-hidden z-[9999] font-cairo">
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            ...
            <div className="flex gap-3">
               ...
               {/* زر الحفظ مرتبط بالدالة الجديدة */}
               <Button onClick={() => bulkSave.mutate(previewQuestions)} disabled={bulkSave.isPending} className="gradient-primary text-white font-black px-12 rounded-xl shadow-xl">
                 {bulkSave.isPending ? <Loader2 className="animate-spin" /> : "حفظ الكل"}
               </Button>
            </div>
          </DialogHeader>
                    ...
                    {['A', 'B', 'C', 'D'].map(letter => (
                      <div key={letter} className="space-y-2 font-cairo">
                        <div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black tracking-widest text-slate-500 uppercase">خيار ({letter})</Label><button onClick={() => {
                            ...
                            setPreviewQuestions(up);
                        }} className={cn("text-[9px] font-black px-2.5 py-1 rounded-lg transition-all", q.correct_option === letter ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400 hover:bg-slate-300")}>{q.correct_option === letter ? 'إجابة صحيحة ✅' : 'تحديد كإجابة'}</button></div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-cairo font-bold">
          ...
          <form onSubmit={(e) => { e.preventDefault(); bulkSave.mutate([formData]); setIsDialogOpen(false); }} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {['a', 'b', 'c', 'd'].map(l => <Input key={l} placeholder={`خيار (${l.toUpperCase()})`} value={formData[`option_${l}` as keyof typeof formData]} onChange={(e) => setFormData({ ...formData, [`option_${l}`]: e.target.value })} className="rounded-xl h-12" />)}
            </div>
            <Button type="submit" disabled={bulkSave.isPending} className="gradient-primary text-white w-full h-12 rounded-xl text-lg font-black shadow-lg">حفظ البيانات</Button>
          </form>
        </DialogContent>
      </Dialog>
    </AdminLayout>
