 ...
 * Version: 19.0 (Universal Parser: Old + New Patterns + Full CRUD)
 */
...
// --- المحرك الذكي الشامل (يدعم النمط القديم + النمط الجديد) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // 1. فحص هل السطر خيار؟ (يدعم النمطين: القديم والجديد)
    // النمط القديم: (1) نص الخيار
    // النمط الجديد: 1) - نص الخيار أو 2) + نص الخيار
    const isOption = t.match(/^\(?\d+[\)\.-]?\s*[-+]/) || // مثل: 1) - أو (1) +
                     t.match(/^\d+\s*\)/) || // مثل: 1)
                     t.startsWith('+') || t.startsWith('-') ||
                     t.includes('العبارة صحيحة') || t.includes('العبارة خاطئة');

    // 2. فحص هل السطر سؤال؟
    // النمط القديم: ينتهي بـ : أو ؟
    const isOldStyleQuestion = t.endsWith(':') || t.includes('؟');
    
    // النمط الجديد: ليس خياراً، وطويل كفاية ليكون سؤالاً (أكثر من 25 حرف مثلاً)
    // هذا يسمح بقبول الجمل السردية كأسئلة
    const isNewStyleQuestion = !isOption && t.length > 25;

    // إذا كان سؤالاً (بأي نمط)
    if (isOldStyleQuestion || isNewStyleQuestion) {
      if (current && current.option_a) results.push(current); // حفظ السابق
      current = {
        id: Math.random().toString(36).substr(2, 9),
        // تنظيف بداية السؤال من الأرقام والأقواس الزائدة
    ...
    // إذا كان خياراً
      ...
      // تحديد الإجابة الصحيحة (علامة + أو كلمة صحيحة إذا كانت + موجودة)
      const isCorrect = t.includes('+');
      
      // تنظيف نص الخيار بدقة من الرموز (+ - 1) )
      let cleanVal = t.replace(/^(\(?\d+[\)\.-]?\s*)?[-+]\s*/, '') // حذف "1) -" أو "+"
                      .replace(/^\(?\d+[\)\.-]?\s*/, '') // حذف الأرقام "1)"
                      .replace(/[-+]/g, '') // حذف أي زائد أو ناقص متبقي
                      .trim();
      
      if (cleanVal) {
        current.count++;
        // تعيين الحروف A, B, C, D
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">العدد: {questions.length} سؤال</p>
          </div>
