 ...
 * Version: 21.2 (Enhanced Multi-Pattern Support)
 */
...
// --- المحرك الذكي المطور لدعم نمط الصورة المرفقة (الترقيم المختلط) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // 1. فحص هل السطر يمثل سؤالاً جديداً (يبدأ برقم متبوع بـ - أو . أو )
    const questionMatch = t.match(/^(\d+)\s*[-\.\)]\s*(.+)/);
    
    // 2. فحص هل السطر يمثل خياراً (يدعم 1. 2. 3. 4. أو أ- ب- أو +1 +2)
    // النمط المتوافق مع الصورة: رقم الخيار متبوع بنقطة أو علامة
    const optionMatch = t.match(/^([1-4]|[أبجد])\s*[-\.\)]\s*(.+)/);

    if (questionMatch) {
      // حفظ السؤال السابق إذا اكتمل
    ...
    else if (current && !t.match(/^\d+/) && t.length > 5) {
      // إذا لم يكن سؤالاً جديداً ولا خياراً، فهو تكملة لنص السؤال
...
const AdminQuestions = () => {
  ...
  const getFriendlyError = (err: any): { title: string; description: string } => {
    ...
    if (err?.code === 'AUTH_REQUIRED') return { title: 'يلزم تسجيل الدخول', description: 'سجّل الدخول بحساب مدير/محرر ثم أعد المحاولة.' };
    if (err?.code === 'FORBIDDEN') return { title: 'لا تملك صلاحية', description: 'هذه العملية متاحة فقط للمديرين أو المحررين.' };
    if (lower.includes('row-level security')) return { title: 'خطأ صلاحيات (RLS)', description: 'حسابك الحالي لا يملك صلاحية التعديل. تأكد من دورك الوظيفي.' };
    return { title: 'حدث خطأ', description: raw || 'حدث خطأ غير متوقع. حاول مرة أخرى.' };
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'تنبيه', description: 'لم يتم العثور على أسئلة بتنسيق مدعوم.', variant: 'destructive' });
      }
    } catch (err) {
      toast({ title: 'خطأ', description: 'حدثت مشكلة أثناء قراءة الملف.', variant: 'destructive' });
    } finally {
  ...
  const bulkSave = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: '✅ تم الاستيراد بنجاح', description: `تمت إضافة ${previewQuestions.length} سؤال.` });
  ...
  const softDeleteMutation = useMutation({
    ...
    onError: (err: any, _ids, ctx) => {
      if (ctx?.previous) ctx.previous.forEach(([key, data]) => queryClient.setQueryData(key, data));
    ...
    onSuccess: (_data, ids) => {
      ...
      if (ids.length > 1) { setSelectedIds([]); setIsBulkDeleteDialogOpen(false); }
    ...
    <AdminLayout>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm">
              <ScanLine className="w-4 h-4" /> استيراد ملف (PDF/صورة)
            </Button>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-5xl bg-white rounded-none md:rounded-3xl p-0 border-none shadow-2xl h-[100dvh] md:h-[94vh] flex flex-col overflow-hidden z-[9999] font-cairo">
          <div className="sticky top-0 z-10 bg-white border-b px-4 md:px-8 py-4 md:py-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <DialogTitle className="text-lg md:text-2xl font-black flex items-center gap-2 md:gap-3 text-slate-900">
                <Eye className="text-primary w-5 h-5 md:w-8 md:h-8" /> مراجعة الاستيراد الذكي
              </DialogTitle>
          ...
          <div className="px-4 md:px-8 py-3">
            <Input placeholder="بحث في النتائج المستخرجة..." value={previewSearch} onChange={(e) => setPreviewSearch(e.target.value)} className="h-10 md:h-12 bg-slate-50 border-slate-100 rounded-2xl font-bold text-sm" />
          </div>
          ...
          <div className="sticky bottom-0 z-10 bg-white border-t px-4 md:px-8 py-4 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
            <Button onClick={() => bulkSave.mutate(previewQuestions)} disabled={bulkSave.isPending} className="gradient-primary text-white font-black w-full h-12 md:h-14 rounded-xl shadow-xl text-base md:text-lg">
              {bulkSave.isPending ? <Loader2 className="animate-spin" /> : `حفظ جميع الأسئلة المستخرجة (${previewQuestions.length})`}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isFileUploadOpen} onOpenChange={setIsFileUploadOpen}>
        <DialogContent className="max-w-md bg-white rounded-3xl p-10 shadow-2xl border-none z-[9999] font-cairo">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="mt-8 border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center hover:border-primary/40 hover:bg-primary/5 cursor-pointer group transition-all relative overflow-hidden">
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept=".pdf,.txt,.html,.jpg,.png,.jpeg" />
            {isProcessingFile ? <div className="flex flex-col items-center gap-4"><Loader2 className="w-12 h-12 text-primary animate-spin" /><p className="text-sm font-black text-slate-600">جاري التحليل الرقمي...</p></div> : <div className="flex flex-col items-center gap-4"><div className="p-5 bg-slate-100 rounded-2xl shadow-sm group-hover:bg-white transition-all"><FileText className="w-10 h-10 text-slate-400 group-hover:text-primary" /></div><p className="font-black text-slate-700 text-lg">اختر صورة أو ملف PDF</p></div>}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteQuestion} onOpenChange={() => setDeleteQuestion(null)}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          ...
          <AlertDialogFooter className="gap-3 mt-8 font-bold">
            ...
            <AlertDialogAction onClick={() => deleteQuestion && softDeleteMutation.mutate([deleteQuestion.id])} disabled={softDeleteMutation.isPending} className="bg-destructive text-white rounded-xl font-black px-10">حذف</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          ...
          <AlertDialogDescription>سيتم حذف كافة الأسئلة المحددة نهائياً من العرض. هل أنت متأكد؟</AlertDialogDescription>
