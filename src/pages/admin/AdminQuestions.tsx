...
const parseSanaaLegalContent = (text: string) => {
  // تنظيف شامل للنص من مخلفات الـ PDF
  ...
  lines.forEach(line => {
    ...
    // 1. التعرف على السؤال: يبدأ بـ (رقم) وينتهي بـ (:)
    ...
    // 2. التعرف على الخيارات: نمط (رقم) - أو (رقم) +
    else if (t.match(/^\(?\d+\)?\s*[-+]/) || t.match(/^\d+\s*\)/)) {
      if (!current) return;
      
      const isCorrect = t.includes('+');
      // استخلاص النص النقي فقط
...
const AdminQuestions = () => {
  ...
  // --- دالة معالجة الـ PDF مع ترتيب إحداثيات النصوص لمنع التداخل ---
  const processPDF = async (file: File) => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      if (textContent.items.length > 0) {
        // ترتيب مكاني: من الأعلى للأسفل (Y) ثم من اليمين لليسار (X) لملفات العربية
        const items: any[] = textContent.items;
        items.sort((a, b) => {
          const yDiff = b.transform[5] - a.transform[5];
          if (Math.abs(yDiff) < 5) return b.transform[4] - a.transform[4]; // نفس السطر، رتب من اليمين
          return yDiff; // سطر مختلف، رتب من الأعلى
        });
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: 'خطأ', description: 'حدثت مشكلة تقنية أثناء قراءة الملف.', variant: 'destructive' });
    } finally {
