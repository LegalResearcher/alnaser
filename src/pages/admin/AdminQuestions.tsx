 ...
 * Version: 14.0 (Stable Worker Fix + Bulk Save Logic)
 */
...
// استيراد المكتبات (تم تعديل الاستيراد لمنع الأخطاء)
import * as pdfjsLib from 'pdfjs-dist';
import Tesseract from 'tesseract.js';

// --- إعداد Worker ثابت لمنع انهيار التطبيق ---
// نستخدم نسخة ثابتة من CDN لضمان التوافق وعدم ظهور "حدث خطأ غير متوقع"
pdfjsLib.GlobalWorkerOptions.workerSrc = `https://unpkg.com/pdfjs-dist@${pdfjsLib.version}/build/pdf.worker.min.js`;

// --- المحرك الذكي (Spatial Parsing) ---
    ...
    <AdminLayout>
      <div className="space-y-6 font-cairo">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold"><ScanLine className="w-4 h-4" /> PDF ذكي</Button>
            <Button size="sm" onClick={() => setIsDialogOpen(true)} disabled={!selectedSubject} className="gradient-primary text-white gap-2 shadow-lg"><Plus className="w-4 h-4" /> إضافة سؤال</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20 font-bold">
          ...
          <Select value={selectedYear || "all"} onValueChange={(v) => setSelectedYear(v === "all" ? "" : v)}><SelectTrigger className="bg-white rounded-xl"><SelectValue placeholder="السنة" /></SelectTrigger><SelectContent className="z-[9999] bg-white shadow-2xl"><SelectItem value="all">الكل</SelectItem>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}</SelectContent></Select>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-bold">
          <DialogHeader><DialogTitle className="text-2xl font-black">إضافة/تعديل سؤال</DialogTitle></DialogHeader>
          <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(formData); }} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Select value={formData.correct_option} onValueChange={(v) => setFormData({...formData, correct_option: v as any})}><SelectTrigger><SelectValue placeholder="الصحيحة" /></SelectTrigger><SelectContent><SelectItem value="A">أ</SelectItem><SelectItem value="B">ب</SelectItem><SelectItem value="C">ج</SelectItem><SelectItem value="D">د</SelectItem></SelectContent></Select>
              <Select value={formData.exam_year} onValueChange={(v) => setFormData({...formData, exam_year: v})}><SelectTrigger><SelectValue placeholder="السنة" /></SelectTrigger><SelectContent>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}</SelectContent></Select>
            </div>
            <Button type="submit" disabled={saveMutation.isPending} className="gradient-primary text-white w-full rounded-xl h-12">حفظ</Button>
          </form>
        </DialogContent>
      </Dialog>
