 ...
 * Version: 5.0 (Final Fix - Supports PDF, Text, HTML with Smart Parser)
 */
...
// استيراد مكتبة PDF.js (يجب التأكد من تثبيتها في المشروع)
import * as pdfjsLib from 'pdfjs-dist';
pdfjsLib.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsLib.version}/pdf.worker.min.js`;

// --- محرك الاستخراج الذكي (Pattern: Sana'a University) ---
const parseLegalExamContent = (textContent: string) => {
  ...
  lines.forEach((line) => {
    ...
    if (text.endsWith(':') || text.includes('؟')) {
      if (currentQ && currentQ.option_a) extracted.push(currentQ);
      currentQ = {
...
const AdminQuestions = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'لم يتم العثور على أسئلة بالنظام المطلوب', variant: 'destructive' });
      }
    } catch (err) {
      toast({ title: 'خطأ في معالجة الملف', variant: 'destructive' });
    } finally {
  ...
  const bulkSave = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'تم استيراد الأسئلة بنجاح ✅' });
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام الاستيراد الذكي (PDF / نصوص)</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20">
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-400 uppercase mr-1">المستوى</Label>
          ...
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-400 uppercase mr-1">السنة</Label>
            <Select value={selectedYear || "all"} onValueChange={(v) => setSelectedYear(v === "all" ? "" : v)}>
              ...
              <SelectContent className="z-[9999] bg-white shadow-2xl"><SelectItem value="all">كافة السنوات</SelectItem>{EXAM_YEARS.map(y => <SelectItem key={y} value={y.toString()}>نموذج {y}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-400 uppercase mr-1">بحث</Label>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-5xl bg-white rounded-3xl p-8 border-none shadow-2xl h-[94vh] flex flex-col overflow-hidden z-[9999]">
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            <div>
              <DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900 font-cairo"><Eye className="text-primary w-8 h-8" /> مراجعة الأسئلة المستخرجة</DialogTitle>
              <p className="text-xs text-slate-400 font-bold tracking-tight">لديك ({previewQuestions.length}) سؤالاً. يمكنك التعديل قبل الحفظ.</p>
            </div>
            <div className="flex gap-3">
               ...
               <Button onClick={() => bulkSave.mutate(previewQuestions)} disabled={bulkSave.isPending} className="gradient-primary text-white font-black px-12 rounded-xl shadow-xl">
                 {bulkSave.isPending ? <Loader2 className="animate-spin mr-2" /> : <Save className="mr-2 w-4 h-4" />} تأكيد وحفظ الكل
               </Button>
            </div>
          </DialogHeader>
                    ...
                    {['A', 'B', 'C', 'D'].map((letter) => {
                      ...
                      return (
                        <div key={letter} className="space-y-2 font-cairo">
                          <div className="flex items-center justify-between px-1"><Label className="text-[10px] font-black text-slate-400 tracking-widest">الخيار ({letter})</Label><button onClick={() => updatePreview(q.id, 'correct_option', letter)} className={cn("text-[9px] font-black px-2.5 py-1 rounded-lg transition-all", isCorrect ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400 hover:bg-slate-300")}>{isCorrect ? 'الإجابة الصحيحة ✅' : 'تحديد كإجابة'}</button></div>
