 ...
 * Version: 12.0 (Final Fix: Robust Bulk Save & Error Tracking)
 */
...
const AdminQuestions = () => {
  ...
  // --- دالة حفظ الكل المحدثة مع تتبع الأخطاء ---
  const bulkSaveMutation = useMutation({
    mutationFn: async (list: any[]) => {
      if (!selectedSubject) throw new Error("يرجى اختيار المادة أولاً");
      if (!user?.id) throw new Error("يجب تسجيل الدخول لإتمام العملية");

      // تجهيز البيانات بدقة لتتوافق مع Supabase
      ...
      console.log("جاري إرسال البيانات:", formatted);
    ...
    onSuccess: () => {
      ...
      toast({ 
        ...
        description: `تمت إضافة ${previewQuestions.length} سؤال إلى بنك الأسئلة.`,
        variant: 'default'
      });
    ...
    onError: (err: any) => {
      toast({ 
        title: 'فشل الحفظ ❌', 
        description: `السبب: ${err.message || "خطأ في قاعدة البيانات"}`, 
        variant: 'destructive' 
      });
    }
  });
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام "الناصر تِك" -Bulk Save Optimized</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm">
              <ScanLine className="w-4 h-4" /> استيراد PDF ذكي
            </Button>
          ...
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            ...
            <div className="flex gap-3">
               ...
               >
                 ...
                 {bulkSaveMutation.isPending ? "جاري الحفظ..." : "حفظ الكل"}
               </Button>
            </div>
          </DialogHeader>
            ...
            <div className="space-y-10 pb-12">
              {previewQuestions.map((q) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 relative group/item hover:bg-white transition-all">
                  ...
                  }} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[100px] font-cairo shadow-sm" dir="rtl" />
                    ...
                    {['A', 'B', 'C', 'D'].map(letter => (
                      <div key={letter} className="space-y-2 font-cairo">
                        <div className="flex justify-between items-center px-1"><Label className="text-[10px] font-black text-slate-500 uppercase">خيار ({letter})</Label><button onClick={() => {
                            ...
                            setPreviewQuestions(up);
                        }} className={cn("text-[9px] font-black px-3 py-1.5 rounded-lg transition-all shadow-sm", q.correct_option === letter ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-400")}>{q.correct_option === letter ? 'إجابة صحيحة ✅' : 'تحديد كإجابة'}</button></div>
      ...
      <Dialog open={isFileUploadOpen} onOpenChange={setIsFileUploadOpen}>
        <DialogContent className="max-w-md bg-white rounded-3xl p-10 shadow-2xl border-none z-[9999] font-cairo">
          <DialogHeader><DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900"><ScanLine className="text-primary" /> استيراد نماذج الجامعة</DialogTitle></DialogHeader>
