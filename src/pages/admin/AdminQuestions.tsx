 ...
 * Version: 24.1 (Integrated Exam Forms: العام، الموازي، المختلط)
 */
...
// --- تحديث مصفوفة النماذج لتشمل الخيارات المطلوبة ---
const EXAM_FORMS = [
...
const AdminQuestions = () => {
  ...
  const [selectedForm, setSelectedForm] = useState<string>(''); // حالة فلترة النماذج
  ...
  const [formData, setFormData] = useState({
    ...
    correct_option: 'A' as any, hint: '', exam_year: '', exam_form: 'General'
  });
  ...
  const { data: questions = [], isLoading } = useQuery({
    queryKey: ['questions', selectedSubject, selectedYear, selectedForm, searchQuery],
    queryFn: async () => {
      ...
      if (selectedForm && selectedForm !== "all") query = query.eq('exam_form', selectedForm);
  ...
  const bulkSave = useMutation({
    mutationFn: async (list: any[]) => {
      ...
      const formatted = list.map(q => ({
        ...
        exam_form: selectedForm || 'General',
        created_by: user.id,
        status: 'active'
      }));
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo font-bold">
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">إدارة بنك الأسئلة</h1>
            <p className="text-sm text-slate-500 font-medium">تحكم كامل بالنماذج (عام/موازي/مختلط)</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20 font-bold font-cairo">
          ...
          {/* فلترة النموذج في الشاشة الرئيسية */}
          <Select value={selectedForm || "all"} onValueChange={setSelectedForm}>
            <SelectTrigger className="bg-white border-slate-200 rounded-xl"><SelectValue placeholder="النموذج" /></SelectTrigger>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden z-10 min-h-[400px]">
          {!selectedSubject ? (
            ...
            <div className="divide-y divide-slate-50 font-cairo font-bold">
              {questions.map((q, i) => (
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <p className="text-slate-800 text-lg leading-relaxed cursor-pointer" onClick={() => toggleSelectOne(q.id)}>{q.question_text}</p>
                        <div className="flex gap-2">
                           <span className="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">
                             {EXAM_FORMS.find(f => f.id === q.exam_form)?.name || 'نموذج العام'}
                           </span>
                           {q.exam_year && <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">سنة {q.exam_year}</span>}
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(q)} className="h-8 w-8 p-0 bg-blue-50 text-blue-600 rounded-lg"><Edit2 className="w-4 h-4" /></Button>
                        <Button variant="ghost" size="sm" onClick={() => setDeleteQuestion(q)} className="h-8 w-8 p-0 bg-red-50 text-red-600 rounded-lg"><Trash2 className="w-4 h-4" /></Button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveSingleMutation.mutate(formData); }} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <div>
                  ...
                  <Input value={formData.hint} onChange={(e) => setFormData({...formData, hint: e.target.value})} className="rounded-xl h-12" placeholder="تلميح اختياري" />
               </div>
            </div>
