 ...
 * Fix: Balanced JSX tags and fixed syntax errors in mapping
 */
...
// --- محرك الاستخراج الذكي المعتمد لنمط جامعة صنعاء ---
const parseLegalExamContent = (textContent: string) => {
  ...
  lines.forEach((line) => {
    ...
    if (text.endsWith(':') || text.includes('؟')) {
      ...
      if (letter) {
        current[`option_${letter.toLowerCase()}`] = cleanValue;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      if (extracted.length > 0) {
        ...
        toast({ title: 'خطأ', description: 'لم يتم العثور على أسئلة بالتنسيق المطلوب', variant: 'destructive' });
      }
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">منصة الناصر تِك - نظام الاستيراد الذكي</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-bold shadow-sm">
              <FileUp className="w-4 h-4" /> رفع ملف الجامعة
            </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100 relative z-20">
          <div className="space-y-1.5">
            ...
            <Select value={selectedLevel} onValueChange={setSelectedLevel}>
              <SelectTrigger className="bg-white border-slate-200 rounded-xl font-bold"><SelectValue placeholder="المستوى" /></SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={selectedSubject} onValueChange={setSelectedSubject}>
              <SelectTrigger className="bg-white border-slate-200 rounded-xl font-bold"><SelectValue placeholder="المادة" /></SelectTrigger>
              <SelectContent className="z-[9999] bg-white border-slate-100 shadow-2xl">
          ...
          <div className="space-y-1.5">
            <Label className="text-[10px] font-black text-slate-400 mr-1 uppercase tracking-widest">السنة</Label>
            <Select value={selectedYear || "all"} onValueChange={(v) => setSelectedYear(v === "all" ? "" : v)}>
              <SelectTrigger className="bg-white border-slate-200 rounded-xl font-bold"><SelectValue placeholder="الكل" /></SelectTrigger>
          ...
          <div className="space-y-1.5">
            <Label className="text-[10px] font-black text-slate-400 mr-1 uppercase tracking-widest">بحث نصي</Label>
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        <div className={cn("p-4 rounded-xl border", q.correct_option === 'A' ? "bg-emerald-50 border-emerald-200 text-emerald-700 font-bold" : "bg-white border-slate-100 text-slate-500")}>أ. {q.option_a}</div>
                        <div className={cn("p-4 rounded-xl border", q.correct_option === 'B' ? "bg-emerald-50 border-emerald-200 text-emerald-700 font-bold" : "bg-white border-slate-100 text-slate-500")}>ب. {q.option_b}</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent className="max-w-5xl bg-white rounded-3xl p-8 border-none shadow-2xl h-[94vh] flex flex-col overflow-hidden z-[9999]">
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            <div className="space-y-1">
              <DialogTitle className="text-2xl font-black flex items-center gap-3 text-slate-900"><Eye className="text-primary w-8 h-8" /> مراجعة الأسئلة المستخرجة</DialogTitle>
              <p className="text-sm text-slate-400 font-bold">لديك ({previewQuestions.length}) سؤالاً جاهزاً للحفظ.</p>
            </div>
            <div className="flex gap-3">
               <Button variant="ghost" onClick={() => setIsPreviewOpen(false)} className="rounded-xl font-bold">إلغاء</Button>
               <Button onClick={() => bulkInsertMutation.mutate(previewQuestions)} disabled={bulkInsertMutation.isPending} className="gradient-primary text-white font-black px-10 rounded-xl shadow-xl">
                 {bulkInsertMutation.isPending ? <Loader2 className="animate-spin mr-2" /> : <Save className="mr-2 w-4 h-4" />} حفظ الكل
               </Button>
            </div>
          </DialogHeader>
          ...
          <ScrollArea className="flex-1 mt-6 pr-4">
            <div className="space-y-10 pb-12">
              {filteredPreview.map((q, idx) => (
                <div key={q.id} className="bg-slate-50/50 p-8 rounded-[36px] border border-slate-100 shadow-sm relative group/item">
                  <button onClick={() => removePreviewQuestion(q.id)} className="absolute -left-3 -top-3 w-10 h-10 bg-white border border-red-100 text-destructive rounded-2xl flex items-center justify-center shadow-lg hover:bg-destructive hover:text-white z-30"><Trash2 className="w-5 h-5" /></button>
                  <Textarea value={q.question_text} onChange={(e) => updatePreviewQuestion(q.id, 'question_text', e.target.value)} className="mb-8 rounded-2xl border-slate-200 font-bold text-lg bg-white min-h-[90px]" dir="rtl" />
                    ...
                    {['A', 'B', 'C', 'D'].map((letter) => {
                      ...
                      return (
                        <div key={letter} className="space-y-2">
                          <div className="flex items-center justify-between px-1"><Label className="text-[10px] font-black text-slate-400">الخيار ({letter})</Label><button onClick={() => updatePreviewQuestion(q.id, 'correct_option', letter)} className={cn("text-[9px] font-black px-2.5 py-1 rounded-lg transition-all", isCorrect ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400 hover:bg-slate-300")}>{isCorrect ? 'الإجابة الصحيحة ✅' : 'تحديد كإجابة'}</button></div>
      ...
      <Dialog open={isFileUploadOpen} onOpenChange={setIsFileUploadOpen}>
        <DialogContent className="max-w-md bg-white rounded-3xl p-10 shadow-2xl border-none z-[9999]">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="group border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center hover:border-primary/40 hover:bg-primary/5 transition-all cursor-pointer relative overflow-hidden">
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept=".txt,.html" />
            {isProcessingFile ? (
              <div className="flex flex-col items-center gap-4"><Loader2 className="w-12 h-12 text-primary animate-spin" /><p className="text-sm font-black text-slate-600">جاري تحليل البيانات...</p></div>
            ) : (
              <div className="flex flex-col items-center gap-4"><div className="p-5 bg-slate-100 rounded-2xl transition-all shadow-sm"><FileText className="w-10 h-10 text-slate-400" /></div><p className="font-black text-slate-700 text-lg tracking-tight">اضغط لاختيار الملف الآن</p></div>
            )}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteQuestion} onOpenChange={() => setDeleteQuestion(null)}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8">
          ...
          <AlertDialogFooter className="gap-3 mt-8">
            ...
            <AlertDialogAction onClick={() => deleteQuestion && supabase.from('questions').update({ status: 'deleted' }).eq('id', deleteQuestion.id).then(() => queryClient.invalidateQueries())} className="bg-destructive text-white rounded-xl font-black px-8">حذف نهائي</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </AdminLayout>
