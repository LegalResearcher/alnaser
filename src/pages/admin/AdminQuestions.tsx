...
// --- مصفوفة النماذج المحدثة (العام، الموازي، المختلط) ---
const EXAM_FORMS = [
...
const AdminQuestions = () => {
  ...
  const [selectedForm, setSelectedForm] = useState<string>(''); // حالة النموذج للفلترة
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <p className="font-bold text-slate-800 text-lg leading-relaxed cursor-pointer" onClick={() => toggleSelectOne(q.id)}>{q.question_text}</p>
                        <div className="flex gap-2">
                           {q.exam_year && <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded text-slate-500 font-bold">عام {q.exam_year}</span>}
                           {q.exam_form && <span className="text-[10px] bg-blue-50 px-2 py-0.5 rounded text-blue-600 font-bold">{EXAM_FORMS.find(f => f.id === q.exam_form)?.name}</span>}
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(q)} className="h-8 w-8 p-0 hover:bg-blue-50 hover:text-blue-600 bg-white border border-slate-100"><Edit2 className="w-4 h-4" /></Button>
