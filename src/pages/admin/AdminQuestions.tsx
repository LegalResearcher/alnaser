 ...
 * Version: 22.0 (Smart Universal Parser + Original Delete Logic from Old Code)
 */
...
// --- المحرك الذكي الشامل (لم يتم تعديله - يدعم كل الأنماط) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // 1. اكتشاف الخيارات
    // يدعم: (1) نص / 1) - نص / + نص / العبارة صحيحة
    const isOption = t.match(/^\(?\d+[\)\.-]?\s*[-+]/) || 
                     t.match(/^\d+\s*\)/) || 
    ...
    const isNewStyleQuestion = !isOption && t.length > 20;
...
const AdminQuestions = () => {
  ...
  // ==========================================================
  // === منطق الحذف (تم استبداله بمنطق الكود القديم) ===
  // ==========================================================
  const deleteMutation = useMutation({
    mutationFn: async (question: Question) => {
      if (role === 'admin') {
        // المسؤول يحذف نهائياً (Hard Delete)
        const { error } = await supabase.from('questions').delete().eq('id', question.id);
        if (error) throw error;
      } else {
        // المستخدم العادي يرسل طلب حذف (Soft Delete / Request)
  ...
  // ==========================================================
  // === منطق الحذف المتعدد (مقتبس من المنطق القديم) ===
  // ==========================================================
  const bulkDeleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (role === 'admin') {
        // المسؤول يحذف المجموعة نهائياً
        const { error } = await supabase.from('questions').delete().in('id', selectedIds);
        if (error) throw error;
      } else {
        // المستخدم العادي يغير الحالة فقط (Soft Delete)
