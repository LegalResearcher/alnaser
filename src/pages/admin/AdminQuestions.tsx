 ...
 * Version: 23.0 (Universal Parser + Robust Old Delete Logic + Fix NULLs)
 */
...
// إعداد Worker الـ PDF
pdfjsLib.GlobalWorkerOptions.workerSrc = `https://unpkg.com/pdfjs-dist@${pdfjsLib.version}/build/pdf.worker.min.js`;

// --- المحرك الذكي الشامل (يدعم كل أنماطك: القديمة، الصح والخطأ، والجديدة +/-) ---
const parseSanaaLegalContent = (text: string) => {
  ...
  lines.forEach(line => {
    ...
    // 1. اكتشاف الخيارات (يدعم كل الأنماط)
    const isOption = t.match(/^[\(\s\d\)\.\-\+]+/) || 
                     t.startsWith('+') || t.startsWith('-') ||
                     t.includes('العبارة صحيحة') || t.includes('العبارة خاطئة');

    // 2. اكتشاف الأسئلة (يدعم التقليدي والسردي الطويل)
    const isOldStyleQuestion = t.endsWith(':') || t.includes('؟');
    const isNewStyleQuestion = !isOption && t.length > 25;

    if (isOldStyleQuestion || isNewStyleQuestion) {
      if (current && current.option_a) results.push(current);
      current = {
        id: Math.random().toString(36).substr(2, 9),
        question_text: t.replace(/^(\(?\d+[\)\.-]?\s*)/, '').trim(),
        option_a: '', option_b: '', option_c: '', option_d: '',
        correct_option: 'A', count: 0
      };
    } 
    else if (isOption) {
      if (!current) return;
...
const AdminQuestions = () => {
  // جلب دور المستخدم (Role) من نظام المصادقة لتطبيق منطق الحذف القديم
  ...
  // --- 2. حفظ فردي (إضافة وتعديل - مع إصلاح NULL) ---
  const saveMutation = useMutation({
    mutationFn: async (data: typeof formData & { subject_id?: string; id?: string }) => {
      ...
      if (editingQuestion) {
        ...
        const { error } = await supabase.from('questions').insert({ ...payload, subject_id: selectedSubject, created_by: user?.id, status: 'active' });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  // =========================================================
  // === تعديل: منطق الحذف مأخوذ حرفياً من الكود القديم الخاص بك ===
  // =========================================================
  const deleteMutation = useMutation({
    mutationFn: async (question: Question) => {
      if (role === 'admin') {
        // إذا كان مسؤولاً: حذف نهائي من القاعدة
        const { error } = await supabase.from('questions').delete().eq('id', question.id);
        if (error) throw error;
      } else {
        // إذا كان محرراً: إرسال طلب حذف وتغيير الحالة
  ...
  // منطق الحذف المتعدد (مبني على نفس منطق الحذف القديم)
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questions'] });
      toast({ title: `تم معالجة حذف ${selectedIds.length} سؤال` });
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">نظام "الناصر تِك" الشامل</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setIsHtmlImportDialogOpen(true)} disabled={!selectedSubject} className="gap-2"><FileCode className="w-4 h-4 text-orange-500" /> HTML</Button>
            <Button variant="outline" size="sm" onClick={() => setIsImportDialogOpen(true)} disabled={!selectedSubject} className="gap-2"><FileSpreadsheet className="w-4 h-4 text-emerald-600" /> Excel</Button>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" onClick={() => { setEditingQuestion(q); setFormData({ question_text: q.question_text, option_a: q.option_a, option_b: q.option_b, option_c: q.option_c || '', option_d: q.option_d || '', correct_option: q.correct_option as any, hint: q.hint || '', exam_year: q.exam_year?.toString() || '' }); setIsDialogOpen(true); }} className="h-9 w-9 p-0 bg-blue-50 text-blue-600 rounded-lg"><Edit2 className="w-4 h-4" /></Button>
                        <Button variant="ghost" size="sm" onClick={() => setDeleteQuestion(q)} className="h-9 w-9 p-0 bg-red-50 text-red-600 rounded-lg"><Trash2 className="w-4 h-4" /></Button>
                      </div>
                    </div>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-5xl bg-white rounded-3xl p-8 border-none shadow-2xl h-[94vh] flex flex-col overflow-hidden z-[9999] font-cairo">
          <DialogHeader className="border-b pb-6 flex flex-row items-center justify-between">
            ...
            <div className="flex gap-3"><Button variant="ghost" onClick={() => setIsPreviewOpen(false)} className="rounded-xl font-bold px-8">إلغاء</Button><Button onClick={() => bulkSaveMutation.mutate(previewQuestions)} disabled={bulkSaveMutation.isPending} className="gradient-primary text-white font-black px-12 rounded-xl shadow-xl">{bulkSaveMutation.isPending ? <Loader2 className="animate-spin" /> : "حفظ الكل"}</Button></div>
          </DialogHeader>
      ...
      {/* Delete Confirmation (The Fixed Part) */}
      <AlertDialog open={!!deleteQuestion} onOpenChange={() => setDeleteQuestion(null)}>
        <AlertDialogContent className="rounded-3xl z-[9999] bg-white p-8 border-none shadow-2xl font-cairo">
          <AlertDialogHeader><AlertDialogTitle className="font-black text-2xl text-slate-900">{role === 'admin' ? 'تأكيد الحذف النهائي' : 'إرسال طلب حذف'}</AlertDialogTitle>
          <AlertDialogDescription className="font-medium text-slate-600">{role === 'admin' ? 'هل أنت متأكد؟ لا يمكن التراجع عن هذا الإجراء.' : 'سيتم إرسال طلب للمسؤول لحذف هذا السؤال.'}</AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-white rounded-3xl p-8 z-[9999] font-cairo font-bold">
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(formData); }} className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">{['a', 'b', 'c', 'd'].map(l => <div key={l}><Label className="text-xs mb-1 block">خيار {l.toUpperCase()}</Label><Input placeholder={`نص الخيار...`} value={formData[`option_${l as keyof typeof formData}` as any]} onChange={(e) => setFormData({ ...formData, [`option_${l}`]: e.target.value })} className="rounded-xl h-12" /></div>)}</div>
            ...
            <div className="mt-4"><Label>ملاحظة (تلميح للطالب)</Label><Input value={formData.hint} onChange={(e) => setFormData({...formData, hint: e.target.value})} className="rounded-xl h-12" placeholder="اختياري" /></div>
