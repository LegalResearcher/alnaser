 ...
 * Version: 23.0 (Universal Parser: Narrative + Marked +/- Patterns + Full CRUD)
 */
...
// رابط الـ Worker الثابت
pdfjsLib.GlobalWorkerOptions.workerSrc = `https://unpkg.com/pdfjs-dist@${pdfjsLib.version}/build/pdf.worker.min.js`;

// --- المحرك الذكي الشامل المطور ---
const parseSanaaLegalContent = (text: string) => {
  ...
  // دالة لاكتشاف هل السطر عبارة عن خيار (أ، ب، ج، د أو 1، 2، 3 مع علامات + / -)
  const isOptionLine = (t: string) => {
    return (
      t.match(/^\(?\d+[\)\.-]?\s*[-+]/) || // مثل: 1) - أو (1) +
      t.match(/^\d+\s*\)/) || // مثل: 1)
      t.startsWith('+') || 
      t.startsWith('-') ||
  ...
  lines.forEach(line => {
    ...
    if (isOptionLine(t)) {
      ...
      // تحديد الإجابة الصحيحة من علامة +
      const isCorrect = t.includes('+');
      
      // تنظيف نص الخيار من الأرقام والأقواس والعلامات (+ - 1 ) . )
      let cleanVal = t
        .replace(/^[\(\s\d\)\.\-\+]+/, '') // حذف البادئة مثل "1) - "
        .replace(/[\+\-]$/, '') // حذف العلامات في نهاية السطر إن وجدت
        .trim();
      ...
      // إذا لم يكن خياراً، فهو سؤال (سواء كان قديماً ينتهي بـ : أو جديداً سردياً)
      
      // إذا كان هناك سؤال مكتمل سابقاً، نقوم بحفظه
      ...
      if (!current) {
        current = {
          ...
          // تنظيف نص السؤال من أي أرقام ترقيم في البداية
        ...
        // دمج الأسطر إذا كان السؤال طويلاً جداً ومقسماً على عدة أسطر
...
const AdminQuestions = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['questions'] });
      toast({ title: role === 'admin' ? 'تم حذف السؤال نهائياً' : 'تم إرسال طلب الحذف' });
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 font-cairo">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">العدد الكلي: {questions.length}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setIsFileUploadOpen(true)} disabled={!selectedSubject} className="gap-2 border-primary/20 bg-primary/5 text-primary font-bold shadow-sm"><ScanLine className="w-4 h-4" /> PDF شامل</Button>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(q)} className="h-9 w-9 p-0 bg-blue-50 text-blue-600 rounded-lg"><Edit2 className="w-4 h-4" /></Button>
