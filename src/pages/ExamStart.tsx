            ...
            <div className="bg-white rounded-[3.5rem] border border-slate-200 shadow-2xl shadow-slate-200/60 overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              <div className="bg-slate-900 p-10 md:p-14 text-white text-center relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  {subject.author_name && (
                    <div className="inline-flex items-center gap-2 text-slate-400 text-sm font-bold bg-white/5 px-4 py-2 rounded-2xl">
                      <User className="w-4 h-4" />
                      بإشراف: {subject.author_name}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-8 md:p-14 space-y-10">
                ...
                {/* Stats Grid - بطاقات المعلومات الصغيرة */}
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-slate-50 rounded-[2.5rem] p-6 text-center border border-slate-100 group hover:border-primary/20 transition-all">
                    <Target className="w-6 h-6 text-primary mx-auto mb-3 group-hover:scale-110 transition-transform" />
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                       ...
                       <Input
                        id="name"
                        placeholder="أدخل اسمك الرباعي للمسودة"
                        value={studentName}
                        onChange={(e) => setStudentName(e.target.value)}
                        className="h-16 rounded-[1.5rem] pr-12 bg-slate-50 border-slate-100 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all text-lg font-bold"
                      />
                    </div>
                  </div>
                  ...
                  {/* الترحيب التفاعلي */}
                  {studentName.trim() && (
                    <div className="bg-primary/5 border border-primary/10 rounded-[2rem] p-6 flex items-center gap-4 animate-in zoom-in-95 duration-300">
                      <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm">
                         <Sparkles className="w-6 h-6 text-primary animate-pulse" />
                      </div>
                      <div>
                        <p className="text-slate-800 font-black">أهلاً بك، {studentName.split(' ')[0]}</p>
                        <p className="text-slate-500 text-xs font-medium">نظام الاختبارات جاهز لاستقبالك.</p>
                      </div>
                    </div>
                  )}

                  {/* Year Selection & Question Info */}
                  <div className="grid md:grid-cols-2 gap-6 items-end">
                    <div className="space-y-3">
                      <Label className="text-sm font-black text-slate-700 mr-2 uppercase tracking-wide">السنة الدراسية (النموذج)</Label>
                      <Select value={selectedYear} onValueChange={setSelectedYear}>
                        ...
                        <SelectContent className="rounded-2xl border-slate-200">
                          {EXAM_YEARS.map((year) => (
                            <SelectItem key={year} value={year.toString()} className="h-12 rounded-xl">
                              دورة عام {year}
                            </SelectItem>
                          ))}
                        </SelectContent>
                    ...
                    <div className="bg-slate-900 rounded-[1.5rem] p-5 h-16 flex items-center justify-between px-6 shadow-xl shadow-slate-200">
                      <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">الأسئلة المتوفرة</span>
                  ...
                  {subject.password && (
                    <div className="space-y-3 p-6 bg-amber-50 rounded-[2rem] border border-amber-100">
                      ...
                      <Input
                        id="password"
                        type="password"
                        placeholder="أدخل كلمة السر لبدء المحاولة"
                        value={password}
                  ...
                  {subject.allow_time_modification && (
                    <div className="space-y-6 pt-4">
                      <div className="flex items-center justify-between px-2">
                        ...
                        <span className="bg-primary text-white px-4 py-1 rounded-full text-xs font-black">{examTime} دقيقة</span>
                      </div>
                      <Slider
                        value={[examTime]}
              ...
              <div className="p-8 bg-slate-50 border-t border-slate-100 text-center flex items-center justify-center gap-2">
                 ...
                 <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
                   عند الضغط على ابدأ، سيتم تفعيل المؤقت الزمني تلقائياً. تأكد من استقرار اتصالك بالإنترنت.
                 </p>
              </div>
            </div>
          </div>
