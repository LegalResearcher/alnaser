...
const ExamPage = () => {
  ...
  // الـ Paywall يعمل في وضع المراجعة فقط إذا لم يكن لديه كلمة مرور صحيحة
  // مشترك دفع وأدخل كلمة مرور → hasValidPassword = true → لا Paywall
  // لم يشترك بعد → hasValidPassword = false/undefined → Paywall بعد 5 أسئلة
  const paywallActive = reviewMode && !state?.hasValidPassword;
  ...
  const { data: questions = [], isLoading } = useQuery({
    queryKey: ["exam-questions", subjectId, state?.examYear, state?.examForm, state?.questionsCount, state?.forcedQuestionIds?.join(","), !!state?.resumeProgress, paywallActive],
    queryFn: async () => {
      ...
      // ── للمستخدم غير المشترك: اجلب أول 5 أسئلة بها hint و explanation فقط ──
      if (paywallActive) {
        const richQ = allQ.filter(q => q.hint && q.hint.trim() && q.explanation && q.explanation.trim());
        return richQ.slice(0, FREE_QUESTIONS_LIMIT);
      }
  ...
  const goTo = (index: number) => {
    ...
    if (paywallActive && index >= FREE_QUESTIONS_LIMIT) {
      setShowPaywall(true);
      return;
    }
  ...
  const goNext = () => {
    ...
    if (paywallActive && nextIndex >= questions.length) {
      setShowPaywall(true);
      return;
    }
    goTo(Math.min(questions.length - 1, nextIndex));
  };
            ...
            {currentQuestion && (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-5 md:mb-8 flex-wrap">
                  ...
                  {/* بادج آخر سؤال مجاني */}
                  {paywallActive && currentIndex === questions.length - 1 && (
                ...
                {hasAnswered && (
                  <div ref={hintSectionRef} className="mt-5 md:mt-8 anim-zoom-in">
                    ...
                    {/* تلميح: زر الشرح المفصل متاح فقط في وضع المراجعة */}
                    {selectedAnswer && currentQuestion.explanation && !reviewMode && (
                      <div className="mb-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        <div className="relative overflow-hidden rounded-2xl border border-amber-200 dark:border-amber-800/50 bg-gradient-to-l from-amber-50 via-yellow-50 to-amber-50 dark:from-amber-950/30 dark:via-yellow-950/30 dark:to-amber-950/30 p-3.5">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-amber-900 dark:text-amber-200 font-black text-xs sm:text-sm leading-relaxed">
                                الشرح المفصل لهذا السؤال متاح حصريًا في <span className="text-amber-700 dark:text-amber-300">وضع اختبار+ المراجعة</span>
                              </p>
                              <p className="text-amber-700/80 dark:text-amber-400/80 text-[10px] sm:text-[11px] font-semibold mt-0.5">
                                فعّل وضع المراجعة من صفحة بدء الاختبار للوصول إلى الشروحات القانونية الكاملة.
                              </p>
                            </div>
                          </div>
                        </div>
        ...
        <div className="max-w-2xl mx-auto flex justify-between items-center gap-3">
          ...
          <div className="flex gap-1.5 items-center">
            {questions.slice(dotStart, dotEnd).map((_, i) => {
              const ri = dotStart + i;
              const isLocked = paywallActive && ri >= FREE_QUESTIONS_LIMIT;
              return (
              ...
              <div className="flex items-center justify-center gap-2 mb-7 bg-indigo-50 dark:bg-indigo-950/40 rounded-2xl px-4 py-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-black text-indigo-700 dark:text-indigo-300">
                    أسئلة اختبارية إضافية متاحة للمشتركين
                  </p>
                </div>
              </div>
