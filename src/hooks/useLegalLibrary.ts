...
export function useLegalAccessMode() {
  ...
  const trialDates = (() => {
    try {
      const startIso = localStorage.getItem('legal_library_trial_start_v1');
      if (!startIso) return null;
      const start = new Date(startIso);
      const end = new Date(start);
      end.setDate(end.getDate() + 3); // TRIAL_DAYS — مطابق لـ LegalWelcomeOnboarding
      return { start, end };
    } catch { return null; }
  })();

  const trialExpired = !!trialDates && trialDates.end.getTime() < Date.now();
  const shouldShowTrialEndedScreen = checked && !isPremiumUnlocked && trialExpired && !isLimitedMode;
...
export function useLegalDocumentContent(id: number | null, enabled: boolean, limitedMode: boolean = false) {
  return useQuery({
    queryKey: ['legal_document_content', id, limitedMode],
    queryFn: async () => {
...
export function useJudicialRuleContent(id: number | null, enabled: boolean, limitedMode: boolean = false) {
  return useQuery({
    queryKey: ['judicial_rule_content', id, limitedMode],
    queryFn: async () => {
      if (id == null) return '';
